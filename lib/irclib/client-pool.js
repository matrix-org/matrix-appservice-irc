...
function checkClientLimit(server) {
    ...
    stats.ircClients(server.domain, numConnectedNicks);

    if (numConnectedNicks < server.getMaxClients()) {
        // under the limit, we're good for now.
        log.debug(
            "%s active connections on %s",
            numConnectedNicks, server.domain
        );
        return;
    }
    ...
    log.debug(
        ...
        numConnectedNicks, server.domain, server.getMaxClients()
    );
