...
var onClientDisconnected = function(virtualUser) {
    ...
    // Reconnect this user
    log.debug(
        "Reconnecting %s@%s in %sms",
        virtualUser.nick, virtualUser.server.domain, RECONNECT_TIME_MS
    );
    ...
    setTimeout(function() {
        cli.connect(callbacks).done(function() {
            log.info("Reconnected %s@%s", cli.nick, cli.server.domain);
        }, function(e) {
            log.error("Failed to reconnect %s@%s", cli.nick, cli.server.domain);
        });
