 ...
 * @return {IrcasClient} A new IrcasClient instance.
 */
IrcServer.prototype.connectAs = function(connectionOpts, callbacks, existingDefer) {
    ...
    clientConnection.create(this, {
        ...
        if (clientIsBot) {
            ...
            ircasClient.addListener("part", function(chan, nick, reason, msg) {
                callbacks.onPart(thisServer, nick, chan, "part");
            });
            ircasClient.addListener("quit", function(nick, reason, chans, msg) {
                chans = chans || [];
                chans.forEach(function(chan) {
                    callbacks.onPart(thisServer, nick, chan, "quit");
                });
            });
            ircasClient.addListener("kick", function(chan, nick, by, reason, msg) {
                callbacks.onPart(thisServer, nick, chan, "kick");
            });
            ircasClient.addListener("join", function(chan, nick, msg) {
            ...
            ircasClient.addListener("names", function(chan, names, msg) {
                if (names) {
            ...
            ircasClient.addListener("+mode", function(channel, by, mode, arg) {
                callbacks.onMode(thisServer, channel, by, mode, true, arg);
            });
            ircasClient.addListener("-mode", function(channel, by, mode, arg) {
                callbacks.onMode(thisServer, channel, by, mode, false, arg);
            });
            ircasClient.addListener("message", function(from, to, text) {
                callbacks.onMessage(
                    thisServer, from, to, actions.irc.createMessage(text)
                );
            });
            ircasClient.addListener("ctcp-privmsg", function(from, to, text) {
                if (text.indexOf("ACTION ") === 0) {
                    callbacks.onMessage(thisServer, from, to, actions.irc.createEmote(
                        text.substring("ACTION ".length)
                    ));
                }
            });
            ircasClient.addListener("notice", function(from, to, text) {
                if (from) { // ignore server notices
                    callbacks.onMessage(
                        thisServer, from, to, actions.irc.createNotice(text)
                    );
                }
            });
            ircasClient.addListener("topic", function(channel, topic, nick) {
                if (nick.indexOf("@") !== -1) {
                ...
                callbacks.onMessage(
                    thisServer, nick, channel, actions.irc.createTopic(topic)
                );
            });
        }
        // === Attach client listeners ===
            ...
            ircasClient.addListener("ctcp-privmsg", function(from, to, text) {
                ...
                if (text.indexOf("ACTION ") === 0) {
                    callbacks.onMessage(
                        thisServer, from, to, actions.irc.createEmote(
                            text.substring("ACTION ".length)
                        )
                    );
                }
