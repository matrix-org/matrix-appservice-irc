...
/**
 * Create an IRC AS Client. Wraps the node-irc library to handle connections
 * correctly.
 * @constructor
 * @param {IrcClient} ircClient The new IRC client.
 * @param {string} domain The domain
 */
function IrcasClient(ircClient, domain) {
 ...
 */
IrcasClient.prototype.connect = function() {
    if (this.dead) {
        throw new Error("connect() called on dead client: " + this.client.nick);
    }
    var self = this;
    ...
    setTimeout(function() {
        if (!gotConnectedCallback) {
            log.error(
                ...
                self.client.nick, domain, CONNECT_TIMEOUT_MS
            );
            self.disconnect("timed out on connection");
        }
    }, CONNECT_TIMEOUT_MS);
 ...
 * @param {string} msg - Message to reject with.
 */
IrcasClient.prototype.disconnect = function(msg) {
    if (this.dead) {
        return;
    }
    log.info(
        "disconnect()ing %s@%s - %s", this.client.nick, this.domain, msg
    );
    this.dead = true;
    ...
    if (this.state !== "connected") {
        ...
        self._connectDefer.reject(msg);
    }
};

IrcasClient.prototype._listenForErrors = function() {
    var self = this;
    var domain = self.domain;
    var nick = self.client.nick;
    self.client.addListener("error", function(err) {
        log.error("Server: %s (%s) Error: %s", domain, nick, JSON.stringify(err));
        self.disconnect("error event fired");
    });
    self.client.addListener("netError", function(err) {
        ...
        self.disconnect("netError event fired");
    });
    self.client.addListener("raw", function(msg) {
        ...
        if (msg && (msg.command === "ERROR" || msg.rawCommand === "ERROR")) {
            ...
            self.disconnect("ERROR command received");
        }
    });
};

IrcasClient.prototype._listenForPings = function() {
...
module.exports = {
     ...
     * @return {Promise} Resolves to an IrcasClient or rejects.
     */
    create: function(server, opts, onCreatedCallback) {
        ...
        var retryConnection = function(e) {
            ...
            var ircasClient = new IrcasClient(nodeClient, server.domain);
            onCreatedCallback(ircasClient);
            return ircasClient.connect();
        };
        // try 3 times to connect then give up
        retryConnection().then(returnClient, function() {
            retryConnection().then(returnClient, function() {
                retryConnection().then(returnClient, function() {
