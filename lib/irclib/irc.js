...
IrcLib.prototype.joinBot = function(ircRoom) {
    ...
    return getBotClient(ircRoom.server).then(function(client) {
        ...
        self.log.info("Bot joined channel %s", ircRoom.channel);
    }, function(e) {
...
var loginToServer = function(server) {
    ...
    ircUser.username = "matrixirc";
    var virtualUser = pool.createIrcClient(ircUser, null, true);
    var chansToJoin = [];
    return getChannelsToJoin(server).then(function(channels) {
        ...
        chansToJoin.forEach(function(c) {
            // join a channel every 3 seconds. We stagger them like this to
