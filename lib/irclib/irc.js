...
IrcLib.prototype.getVirtualIrcUser = function(server, userId) {
    if (server.claimsUserId(userId)) {
        // FIXME: This belongs in bridge, not here. It should just return from the pool
        // in this case!
    ...
    var desiredNick = virtualUser.nick;

    virtualUser.connect(globalHooks).done(function() {
        if (desiredNick !== virtualUser.nick) {
            lg.debug("Connected with nick '%s' instead of desired nick '%s'",
                virtualUser.nick, desiredNick);
            pool.updateIrcNick(virtualUser, desiredNick, virtualUser.nick);
        }
