...
IrcLib.prototype.joinBot = function(ircRoom) {
    if (ircRoom.channel.indexOf("#") !== 0) {
        return; // ignore pm rooms
    }
    var self = this;
    getBotClient(ircRoom.server).done(function(client) {
        if (Object.keys(client.chans).indexOf(ircRoom.channel) === -1) {
            self.log.info(
                "Joining bot to %s on %s", ircRoom.channel, ircRoom.server.domain
            );
            client.join(ircRoom.channel, function() {
                self.log.info(
                    "Joined bot to %s on %s", ircRoom.channel, ircRoom.server.domain
                );
            });
        }
    }, function(e) {
        self.log.error("Failed to find bot: %s", e);
    });
};
...
IrcLib.prototype.partBot = function(ircRoom) {
    ...
    var self = this;
    getBotClient(ircRoom.server).done(function(client) {
        client.part(ircRoom.channel, "No bridged users", function() {
            self.log.info("Parted %s from channel %s on %s",
                client.nick, ircRoom.channel, ircRoom.server.domain
            );
        });
    }, function(e) {
        self.log.error("Failed to find bot: %s", e);
...
var loginToServer = function(server) {
    return getChannelsToJoin(server).then(function(channels) {
        ...
        /*
        client.on("netError", function() {
            log.info("Bot network failure: reconnecting in 3s.");
            client.disconnect();
            setTimeout(function() {
...
module.exports.trackChannel = function(server, channel) {
    ...
    getBotClient(server).then(function(client) {
        ...
        client.join(channel, function() {
            log.debug("Joined channel %s", channel);
            client.removeListener("error", failFn);
            var room = new IrcRoom(server, channel);
            defer.resolve(room);
        });
    }, function(err) {
        defer.reject(err);
