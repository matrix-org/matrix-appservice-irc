...
module.exports.sendBotText = function(server, channel, text) {
    var defer = q.defer();
...
module.exports.trackChannel = function(server, channel) {
    ...
    getBotClient(server).then(function(client) {
        client.join(channel, function() {
            var room = roomModels.createIrcRoom(server, channel);
            defer.resolve(room);
        });
    }, function(err) {
        defer.reject(err);
...
protocols.setMapper("users", PROTOCOLS.MATRIX, PROTOCOLS.IRC, function(user) {
