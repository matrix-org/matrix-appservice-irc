...
module.exports.getVirtualIrcUser = function(server, userId) {
    ...
    var ircUser = users.irc.createUser(server, server.nickPrefix + nick, true);
...
module.exports.getPrivateServers = function() {
    var privateServers = [];
    for (var i=0; i<servers.length; i++) {
        var server = servers[i];
        // private servers expose their channels for authed users.
        if (server.clientAuthUrl && server.expose.channels) {
