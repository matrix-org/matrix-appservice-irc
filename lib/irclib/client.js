...
VirtualIrcUser.prototype.connect = function(hooks) {
    var that = this;
    return this.server.connectAs({
        nick: this.nick,
        username: this.userId
    }, hooks).then(function(ircasClient) {
        that.client = ircasClient.client;
        pool.onConnected(that);
        // we may have been assigned a different nick, so update it from source
        that.nick = ircasClient.client.nick;
        ...
        ircasClient.client.addListener("registered", function() {
        ...
        ircasClient.client.addListener("nick", function(old, newNick) {
            if (old === that.nick) {
