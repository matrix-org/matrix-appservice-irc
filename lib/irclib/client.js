 ...
 * @param {Bridge} bridge : The matrix-appservice-bridge Bridge instance.
 */
function BridgedClient(ircUser, matrixUser, isBot, bridge) {
    this.bridge = bridge;
...
BridgedClient.prototype.setIrcUserInfo = function(ircUser) {
    ...
    this.userId = this.matrixUser ? this.matrixUser.getId() : ircUser.username;
};
...
BridgedClient.prototype._onConnectionCreated = function(connInst, nameInfo, callbacks) {
    ...
    clientEvents.addHooks(this, connInst, callbacks, this.bridge);
};
module.exports.BridgedClient = BridgedClient;

function setTopic(bcli, room, topic) {
    // join the room if we haven't already
    var defer = promiseutil.defer();
    joinChannel(bcli, room.channel).done(function() {
        bcli.log.info("Setting topic to %s in channel %s", topic, room.channel);
        bcli.unsafeClient.send("TOPIC", room.channel, topic);
        defer.resolve();
    }, function(e) {
        defer.reject(e);
    });
...
function sendMessage(bcli, room, msgType, text) {
    // join the room if we haven't already
    var defer = promiseutil.defer();
    msgType = msgType || "message";
    bcli._connectDefer.promise.then(function() {
        return joinChannel(bcli, room.channel);
    }).done(function() {
        if (msgType == "action") {
            bcli.unsafeClient.action(room.channel, text);
        }
        else if (msgType == "notice") {
            bcli.unsafeClient.notice(room.channel, text);
        }
        else if (msgType == "message") {
            bcli.unsafeClient.say(room.channel, text);
        }
        defer.resolve();
    }, function(e) {
        bcli.log.error("sendMessage: Failed to join channel " + room.channel);
        defer.reject(e);
    });
    return defer.promise;
}

function joinChannel(bcli, channel, attemptCount) {
    attemptCount = attemptCount || 1;
    if (!bcli.unsafeClient) {
        return Promise.reject("No client");
    }
    if (Object.keys(bcli.unsafeClient.chans).indexOf(channel) !== -1) {
        return Promise.resolve(new IrcRoom(bcli.server, channel));
    }
    if (channel.indexOf("#") !== 0) {
        // PM room
        return Promise.resolve(new IrcRoom(bcli.server, channel));
    }
    if (bcli.server.isExcludedChannel(channel)) {
        return Promise.reject(channel + " is a do-not-track channel.");
    }
    var defer = promiseutil.defer();
    bcli.log.debug("Joining channel %s", channel);
    bcli._addChannel(channel);
    var client = bcli.unsafeClient;
    ...
    var failFn = function(err) {
        ...
        bcli.log.error("Join channel %s : %s", channel, err);
        if (failCodes.indexOf(err.command) !== -1 &&
                err.args.indexOf(channel) !== -1) {
            bcli.log.error("Cannot track channel %s: %s", channel, err.command);
    ...
    setTimeout(function() {
        ...
        if (defer.promise.isPending() && bcli.chanList.indexOf(channel) !== -1) {
            // we may have joined but didn't get the callback so check the client
            if (Object.keys(bcli.unsafeClient.chans).indexOf(channel) !== -1) {
                // we're joined
                bcli.log.debug("Timed out joining %s - didn't get callback but " +
                    "are now joined. Resolving.", channel);
                defer.resolve(new IrcRoom(bcli.server, channel));
                return;
            }
            if (attemptCount >= 5) {
            ...
            bcli.log.error("Timed out trying to join %s - trying again.", channel);
            // try joining again.
            attemptCount += 1;
            joinChannel(bcli, channel, attemptCount).done(function(s) {
                defer.resolve(s);
            }, function(e) {
                defer.reject(e);
            });
    ...
    bcli.unsafeClient.join(channel, function() {
        bcli.log.debug("Joined channel %s", channel);
        client.removeListener("error", failFn);
        var room = new IrcRoom(bcli.server, channel);
        defer.resolve(room);
    });
