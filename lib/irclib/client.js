 ...
 */
VirtualIrcUser.prototype.connectAs = function(connectionOpts, callbacks) {
    var channels = connectionOpts.channels;
    var server = this.server;
    var nick = connectionOpts.nick;
    var clientIsBot = nick === server.nick;

    var nameInfo = names.getIrcNames(
        this.server, this.nick, connectionOpts.username, this.userId
    );


    var password = clientIsBot ? server.nickPass : undefined;

    // strip do not track channels
    if (channels) {
        for (var i = 0; i < channels.length; i++) {
            if (server.doNotTrackChannels.indexOf(channels[i]) !== -1) {
                channels.splice(i, 1);
                i--;
            }
        }
    ...
    log.info("Connecting to IRC server %s as %s (user=%s)- Joining channels %s",
        server.domain, nameInfo.nick, nameInfo.username, JSON.stringify(channels));
    var defer = q.defer();
    clientConnection.create(server, {
        ...
        if (channels) {
            for (var i = 0; i < channels.length; i++) {
                connInst.client.join(channels[i]);
            }
        }
