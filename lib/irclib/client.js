    ...
    names.getIrcNames(
        ...
        connInst.client.addListener("registered", function() {
            ...
            if (oldNick !== self.unsafeClient.nick) {
                ...
                pool.updateIrcNick(self, oldNick, self.unsafeClient.nick);
            }
        });
...
var sendMessage = function(that, room, msgType, text) {
    ...
    that.connectDefer.promise.then(function() {
        ...
        if (msgType == "action") {
            that.unsafeClient.action(room.channel, text);
        }
        else if (msgType == "notice") {
            that.unsafeClient.notice(room.channel, text);
        }
        else if (msgType == "message") {
            that.unsafeClient.say(room.channel, text);
        }
        defer.resolve();
    });
    return defer.promise;
