...
var BridgeRequest = require("../models/req");
var log = require("../logging").get("irc-client-events");

var CLEANUP_TIME_MS = 1000 * 60 * 10; // 10min
...
module.exports.addHooks = function(client, connInst, callbacks, bridge) {
    ...
    if (!client.isBot) {
        ...
        connInst.addListener("message", function(from, to, text) {
            ...
            var req = new BridgeRequest(bridge.getRequestFactory().newRequest(), true);
            complete(req, callbacks.onPrivateMessage(
                req,
    ...
    var popName = function() {
        ...
        var req = new BridgeRequest(bridge.getRequestFactory().newRequest(), true);
        return complete(req, callbacks.onJoin(
    ...
    hookIfClaimed(client, connInst, "message", function(from, to, text) {
        if (to.indexOf("#") !== 0) { return; }
        var req = new BridgeRequest(bridge.getRequestFactory().newRequest(), true);
        complete(req, callbacks.onMessage(
    ...
    hookIfClaimed(client, connInst, "notice", function(from, to, text) {
        ...
        if (from) { // ignore server notices
            var req = new BridgeRequest(bridge.getRequestFactory().newRequest(), true);
            complete(req, callbacks.onMessage(
                req, server, createUser(from), to, actions.irc.createNotice(text)
            ));
        }
    });
    hookIfClaimed(client, connInst, "topic", function(channel, topic, nick) {
        ...
        var req = new BridgeRequest(bridge.getRequestFactory().newRequest(), true);
        complete(req, callbacks.onMessage(
            req, server, createUser(nick), channel, actions.irc.createTopic(topic)
        ));
    });
};
