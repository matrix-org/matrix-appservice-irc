...
module.exports.sync = function(server) {
    ...
    getSyncableRooms(server).then(function(syncRooms) {
        ...
        start = Date.now();
        log.info("Joining IRC users to rooms...");
        return joinIrcUsersToRooms(rooms, server);
    }).then(function() {
        log.info("Joined IRC users to Matrix rooms. (%sms)", Date.now() - start);
        start = Date.now();
...
function joinMatrixUsersToChannels(rooms, server) {
    var STAGGER_TIME_MS = 800;
    var d = q.defer();
    var injectControlDefer = q.defer();
    var promises = [injectControlDefer.promise];
    // filter out rooms listed in the rules
    var rules = server.getMembershipListRules();

    function _addNext(roomsIndex, realsIndex) {
        var room = rooms[roomsIndex];
        if (!room) {
            log.debug("No more matrix users to join.");
            injectControlDefer.resolve();
            return;
        }
        // first time adding to this room, check if we should skip this room.
        if (realsIndex === 0) {
            for (var i = 0; i < rules.rooms.length; i++) {
                ...
                if (room.roomId === roomRules.room && !roomRules.matrixToIrc) {
                    ...
                    _addNext(roomsIndex + 1, 0);
                    return;
                }
            }
        }

        var userId = room.reals[realsIndex];
        if (!userId) {
            _addNext(roomsIndex + 1, 0);
            return;
        }
        log.debug("Injecting join event for %s in %s", userId, room.roomId);
        promises.push(injectJoinEvent(room.roomId, userId));
        setTimeout(function() {
            _addNext(roomsIndex, realsIndex + 1);
        }, STAGGER_TIME_MS);
    }
...
function injectJoinEvent(roomId, userId) {
    var target = new MatrixUser(userId, null);
