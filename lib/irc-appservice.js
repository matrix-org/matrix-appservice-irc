...
module.exports.configure = function(opts) {
    ...
    */
    ...
    if (runAuthWebServer) {
        opts.auth = opts.auth || {};
        if (!opts.auth.port || !opts.auth.redirectBase) {
            throw new Error("One or more IRC servers require auth but there is"+
                " no configured top-level auth.redirectBase/port specified. This"+
                " is required in order to listen for incoming auth redirects.");
        }
        authServer.run(opts.auth.redirectBase, opts.auth.port);
    }
...
module.exports.register = function(controller, serviceConfig) {
    ...
    controller.on("type:m.room.member", function(event) {
        if (event.content && event.content.membership === "invite") {
            var inviter = users.matrix.createUser(
                event.user_id, null
            );
            var invitee = users.matrix.createUser(
                event.state_key, null
            );
            return bridge.hooks.matrix.onInvite(event, inviter, invitee);
        }
    });
