 ...
 */
function BridgedClient(ircUser, matrixUser, isBot, eventBroker, identGenerator) {
...
BridgedClient.prototype.connect = function() {
    ...
    this._identGenerator.getIrcNames(this.ircUser, this.matrixUser).then(function(nameInfo) {
        self.log.info(
        ...
        connInst.client.addListener("error", function(err) {
            if (!err || !err.command || connInst.dead) {
                return;
            }
            var msg = "Received an error on " + self.server.domain + ": " + err.command + "\n";
            msg += JSON.stringify(err.args);
            self._eventBroker.sendMetadata(self, msg);
        });
