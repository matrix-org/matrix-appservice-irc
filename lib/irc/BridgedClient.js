 ...
 */
function BridgedClient(server, ircClientConfig, matrixUser, isBot, eventBroker, identGenerator, ipv6Generator) {
    ...
    this.nick = this._getValidNick(ircClientConfig.getDesiredNick() || server.getNick(this.userId, this.displayName), false);
    this.password = (ircClientConfig.getPassword() ? ircClientConfig.getPassword() : server.config.password);
...
BridgedClient.prototype.connect = Promise.coroutine(function* () {
    ...
    try {
        let nameInfo = yield this._identGenerator.getIrcNames(this._clientConfig, this.matrixUser);
        if (this.server.getIpv6Prefix()) {
            // side-effects setting the IPv6 address on the client config
            yield this._ipv6Generator.generate(this.server.getIpv6Prefix(), this._clientConfig);
        }
        this.log.info("Connecting to IRC server %s as %s (user=%s)", server.domain, this.nick, nameInfo.username);
        this._eventBroker.sendMetadata(this, `Connecting to the IRC network '${this.server.domain}' as ${this.nick}...`);
        ...
        this._keepAlive();
        let connectText = (`You've been connected to the IRC network '${this.server.domain}' as ${this.nick}.`);
        ...
        if (userModes.length > 0 && !this.isBot) {
            ...
            connectText += (` User modes +${userModes} have been set.`);
        }
        this._eventBroker.sendMetadata(this, connectText);
        connInst.client.addListener("nick", (old, newNick) => {
            if (old === this.nick) {
                this.log.info("NICK: Nick changed from '" + old + "' to '" + newNick + "'.");
                this.nick = newNick;
...
BridgedClient.prototype.whois = function (nick) {
    ...
    return new Promise(function (resolve, reject) {
        self.unsafeClient.whois(nick, function (whois) {
            ...
            let chans = ((whois.channels && whois.channels.length) > 0 ?
...
BridgedClient.prototype._onConnectionCreated = function (connInst, nameInfo) {
    ...
    connInst.onDisconnect = function () {
        ...
        self._eventBroker.sendMetadata(self, "Your connection to the IRC network '" + self.server.domain +
            "' has been lost. ");
...
BridgedClient.prototype._joinChannel = function (channel, key, attemptCount) {
    ...
    var failFn = (err) => {
        ...
        if (failCodes.indexOf(err.command) !== -1 &&
            ...
            this._eventBroker.sendMetadata(this, `Could not join ${channel} on '${this.server.domain}': ${err.command}`, true);
        }
    };
    ...
    setTimeout(() => {
        if (!this.unsafeClient) {
            log.error(`Could not try to join: no client for ${this.nick}, channel = ${channel}`);
            return;
        }
        ...
        if (defer.promise.isPending() && this.chanList.indexOf(channel) !== -1) {
            ...
            if (attemptCount >= 5) {
                defer.reject(new Error("Failed to join " + channel + " after multiple tries"));
                return;
            }
