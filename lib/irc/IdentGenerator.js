 ...
 * }
 */
IdentGenerator.prototype.getIrcNames = function(ircClientConfig, matrixUser) {
    ...
    info.nick = ircClientConfig.getDesiredNick().replace(
        /[^A-Za-z0-9\]\[\^\\\{\}\-`_]/g, ""
    );
    if (matrixUser) {
        ...
        info.username = sanitiseUsername(
            ircClientConfig.getUsername() // the bridge won't have a mx user
        );
...
IdentGenerator.prototype._checkQueue = Promise.coroutine(function*() {
    if (!this.processing) {
        ...
        var ircClientConfig = this.processing.ircClientConfig;
        ...
        try {
            log.debug(
                ...
                matrixUser.getId(), configDomain, this.queue.length
            );
            ...
            defer.resolve(config.getUsername());
        }
        catch (err) {
            log.error(
                "Failed to generate username for %s",
                matrixUser.getId()
            );
            log.error(err.stack);
            defer.reject(err);
        ...
        log.debug(
            ...
            this.processing.matrixUser.getId(), this.processing.ircClientConfig.getDomain()
        );
    }
});
