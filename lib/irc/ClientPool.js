...
ClientPool.prototype.killAllClients = function() {
    ...
    var killList = domainList.map((domain) => {
        let clients = Object.keys(this._virtualClients[domain].nicks).map(
        ...
        return clients.filter((c) => c !== undefined);
    }).reduce((a, b) => {return a.concat(b)});

    // console.log('Clients to kill ', killList.map((c)=>c.nick));
