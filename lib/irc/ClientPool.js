...
ClientPool.prototype.createIrcClient = function(ircClientConfig, matrixUser, isBot) {
    ...
    bridgedClient.on("nick-add", this._onNickAdd.bind(this));
    bridgedClient.on("nick-remove", this._onNickRemove.bind(this));
...
ClientPool.prototype._onNickAdd = function(bridgedClient, nick) {
    if(this._virtualClients[bridgedClient.server.domain].nicks[nick] == undefined) {
       this._virtualClients[bridgedClient.server.domain].nicks[nick] = bridgedClient; 
    }
};

ClientPool.prototype._onNickRemove = function(bridgedClient, nick) {
    if(this._virtualClients[bridgedClient.server.domain].nicks[nick] == bridgedClient) {
        this._virtualClients[bridgedClient.server.domain].nicks[nick] = undefined;
    }
};
