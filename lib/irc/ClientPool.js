...
ClientPool.prototype.killAllClients = function () {
    ...
    domainList.forEach((domain) => {
        clients = clients.concat(Object.keys(this._virtualClients[domain].nicks).map((nick) => this._virtualClients[domain].nicks[nick]));
        clients = clients.concat(Object.keys(this._virtualClients[domain].userIds).map((userId) => this._virtualClients[domain].userIds[userId]));
        clients.push(this._botClients[domain]);
    });
    clients = clients.filter((c) => Boolean(c));
    return Promise.all(clients.map((client) => client.kill()));
};
ClientPool.prototype.getOrCreateReconnectQueue = function (server) {
    ...
    if (q === undefined) {
        q = this._reconnectQueues[server.domain] = new QueuePool(server.getConcurrentReconnectLimit(), (item) => {
...
ClientPool.prototype.createIrcClient = function (ircClientConfig, matrixUser, isBot) {
    var bridgedClient = this._ircBridge.createBridgedClient(ircClientConfig, matrixUser, isBot);
...
ClientPool.prototype._checkClientLimit = function (server) {
    ...
    oldest.disconnect("Client limit exceeded: " + server.getMaxClients()).then(function () {
...
ClientPool.prototype._getNumberOfConnections = function (server) {
    if (!server || !this._virtualClients[server.domain]) {
        return 0;
    }
...
ClientPool.prototype.getNickUserIdMappingForChannel = function (server, channel) {
    ...
    Object.keys(cliSet).filter((userId) => cliSet[userId] && cliSet[userId].chanList
        && cliSet[userId].chanList.includes(channel)).forEach((userId) => {
...
ClientPool.prototype._onClientDisconnected = function (bridgedClient) {
    ...
    var cli = this.createIrcClient(cliConfig, bridgedClient.matrixUser, bridgedClient.isBot);
...
ClientPool.prototype._onJoinError = Promise.coroutine(function* (bridgedClient, chan, err) {
    ...
    let matrixRooms = yield this._ircBridge.getStore().getMatrixRoomsForChannel(bridgedClient.server, chan);
    let promises = matrixRooms.map((room) => {
        return this._ircBridge.getAppServiceBridge().getIntent().kick(room.getId(), bridgedClient.userId, `IRC error on ${chan}: ${err}`);
    });
