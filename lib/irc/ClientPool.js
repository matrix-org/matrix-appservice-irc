...
var BridgedClient = require("./BridgedClient");
...
ClientPool.prototype.createIrcClient = function(ircUser, matrixUser, isBot, bridge) {
    var bridgedClient = new BridgedClient(ircUser, matrixUser, isBot, bridge);
...
ClientPool.prototype._checkClientLimit = function(server) {
    ...
    this._removeVirtualUser(oldest);
...
ClientPool.prototype._removeVirtualUser = function(virtualUser) {
    var server = virtualUser.server;
    this._virtualClients[server.domain].userIds[virtualUser.userId] = undefined;
    this._virtualClients[server.domain].nicks[virtualUser.nick] = undefined;
}

ClientPool.prototype._onClientConnected = function(virtualUser) {
    var server = virtualUser.server;
    ...
    this._virtualClients[server.domain].nicks[actualNick] = virtualUser;
...
ClientPool.prototype._onClientDisconnected = function(virtualUser) {
    this._removeVirtualUser(virtualUser);
    this._sendConnectionMetric(virtualUser.server);
    ...
    var cli = this.createIrcClient(
        virtualUser.ircUser, virtualUser.matrixUser, virtualUser.isBot, virtualUser.bridge
    );
...
ClientPool.prototype._onNickChange = function(virtualUser, oldNick, newNick) {
    this._virtualClients[virtualUser.server.domain].nicks[oldNick] = undefined;
    this._virtualClients[virtualUser.server.domain].nicks[newNick] = virtualUser;
}
