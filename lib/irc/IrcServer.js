...
/**
 * Returns whether the server is configured to wait some time
 * (see this.config.getDisconnectPresenceTimeoutMs()) before parting
 * a user that has `/quit` and also wait getDeboucePartsMs before parting
 * a user that has disconnected due to a net-split.
 * @return {bool} this.config.debounceParts.
 */
IrcServer.prototype.shouldDebouceParts = function() {
    return this.config.debounceParts;
}

/**
 * Get the number of ms to debounce before bridging a `/quit` to a leave
 * during a detected net-split. If the user rejoins a channel before bridging
 * the quit to a leave, the leave will not be sent.
 * @return {integer}
 */
IrcServer.prototype.getDeboucePartsMs = function() {
    return this.config.debouncePartsMs;
}
