...
function doRateLimited(fn, attempts, timeoutMs) {
        ...
        fn().then(
            ...
            (err) => {
                ...
                if (err.data && err.data.retry_after_ms && attempts > 0) {
                    ...
                    setTimeout(retry, err.data.retry_after_ms + 5000);
                }
            }
        )
    }
    ...
    return defered.promise.timeout(timeoutMs);
}
...
Provisioner.prototype._userHasProvisioningPower = Promise.coroutine(
    function*(userId, roomId) {
        ...
        // Try 10 times to join a room, or timeout after 10 min
        yield doRateLimited(() => {
            return matrixClient.joinRoom(roomId);
        }, 100, 600000);

        try {
            // Try 10 times to get power state in a room, or timeout after 10 min
            powerState = yield doRateLimited(() => {
                return matrixClient.getStateEvent(roomId, 'm.room.power_levels');
            }, 100, 600000);
        }
        catch (err) {
