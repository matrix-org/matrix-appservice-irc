...
// retry(10, matrixClient, getStateEvent, roomId, 'm.room.power_levels');|
...
Provisioner.prototype._userHasProvisioningPower = Promise.coroutine(
    function*(userId, roomId) {
        ...
        yield retry(100, matrixClient, joinRoom, roomId).timeout(600000);

        try {
            // Try 100 times to get power state in a room, or timeout after 10 min
            powerState = yield retry(
                100, matrixClient, getStateEvent, roomId, 'm.room.power_levels'
            ).timeout(600000);
        }
        catch (err) {
