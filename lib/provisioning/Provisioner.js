...
function Provisioner(ircBridge) {
    ...
    if (!(as.app.use && as.app.get && as.app.post)) {
        log.warn('Could not start provisioning API');
        return;
    }

    as.app.use(function(req, res, next) {
        if (req.url === '/_matrix/provision/unlink' ||
    ...
    as.app.get("/_matrix/provision/listlinks/:roomId", Promise.coroutine(function*(req, res) {
        try {
            let list = yield self.listings({matrix_room_id: req.params.roomId});
            res.json(list);
        }
        catch (err) {
            res.status(500).json({error: err.message});
...
Provisioner.prototype.validate = function(obj, parameterName) {
    let actual = obj[parameterName];
...
Provisioner.prototype.validateAll = function(parameters) {
    let parameterNames = ['matrix_room_id', 'remote_room_channel', 'remote_room_server'];
    for (var i = 0; i < parameterNames.length; i++) {
        this.validate(parameters, parameterNames[i]);
    }
};
...
Provisioner.prototype.listings = function(options) {
    this.validate(options, 'matrix_room_id');
