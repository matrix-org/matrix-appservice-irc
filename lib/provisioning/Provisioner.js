...
var ProvisionRequest = require("./ProvisionRequest");
...
function Provisioner(ircBridge, enabled, requestTimeoutSeconds) {
    ...
    let createProvisionEndpoint = (fn, fnName) => {
        return Promise.coroutine(function* (req, res) {
            ...
            req.log.info('New provisioning request: ' + JSON.stringify(req.body) +
    ...
    as.app.post("/_matrix/provision/link", createProvisionEndpoint(this.requestLink, 'requestLink'));
    as.app.post("/_matrix/provision/unlink", createProvisionEndpoint(this.unlink, 'unlink'));
    as.app.get("/_matrix/provision/listlinks/:roomId", createProvisionEndpoint(this.listings, 'listings'));
    as.app.post("/_matrix/provision/querylink", createProvisionEndpoint(this.queryLink, 'queryLink'));
    as.app.get("/_matrix/provision/querynetworks", createProvisionEndpoint(this.queryNetworks, 'queryNetworks'));
    if (enabled) {
...
Provisioner.prototype._updateBridgingState = Promise.coroutine(function* (req, roomId, userId, status, skey) {
...
Provisioner.prototype._userHasProvisioningPower = Promise.coroutine(function* (req, userId, roomId) {
    ...
    try {
        ...
        throw new Error('Room failed validation. You may be attempting to "double bridge" this room.' +
            ' Error: ' + err);
    }
    try {
...
Provisioner.prototype._authoriseProvisioning = Promise.coroutine(function* (req, server, userId, ircChannel, roomId, opNick, key) {
    ...
    try {
        ...
        wholeBridgingState = roomState.find((e) => {
    ...
    if (wholeBridgingState && wholeBridgingState.content) {
        ...
        if (bridgingState.status !== 'failure') {
            ...
            req.log.info(`Bridging state already exists in room ${roomId} ` +
                `(status = ${bridgingState.status},` +
            ...
            if (bridgingState.status === 'success') {
                ...
                if (!entry) {
                    ...
                    req.log.warn(`Bridging state in room states successful mapping, ` +
                ...
                req.log.warn(`Bridging state in room states pending mapping, ` +
    ...
    this._createAuthorisedLink(req, botClient, server, opNick, ircChannel, key, roomId, userId, skey, timeoutSeconds);
});
Provisioner.prototype._sendToUser = Promise.coroutine(function* (receiverNick, server, message) {
    let botClient = yield this._ircBridge.getBotClient(server);
    return this._ircBridge.sendIrcAction(new IrcRoom(server, receiverNick), botClient, new IrcAction("message", message));
});
// Contact an operator, asking for authorisation for a mapping, and if they reply
//  'yes' or 'y', create the mapping.
Provisioner.prototype._createAuthorisedLink = Promise.coroutine(function* (req, botClient, server, opNick, ircChannel, key, roomId, userId, skey, timeoutSeconds) {
    ...
    yield this._sendToUser(opNick, server, `${userId} has requested to bridge ${roomDesc} with ${ircChannel} on this IRC ` +
...
Provisioner.prototype.handlePm = Promise.coroutine(function* (server, fromUser, text) {
    if (['y', 'yes'].indexOf(text.trim().toLowerCase()) == -1) {
        ...
        yield this._sendToUser(fromUser.nick, server, 'Please respond with "yes" or "y".');
        return;
    }
    ...
    if (request) {
        ...
        yield this._sendToUser(fromUser.nick, server, 'Thanks for your response, bridge request authorised.');
        return;
    }
    log.warn(`Provisioner was not expecting PM from ${fromUser.nick} on ${server.domain}`);
    yield this._sendToUser(fromUser.nick, server, 'The bot was not expecting a message from you. You might have already replied to a request.');
});
...
Provisioner.prototype.queryLink = Promise.coroutine(function* (req) {
    ...
    queryInfo.operators = queryInfo.operators.filter((nick) => {
...
Provisioner.prototype._doLink = Promise.coroutine(function* (req, server, ircChannel, key, roomId, userId) {
    let ircDomain = server.domain;
    let mappingLogId = `${roomId} <---> ${ircDomain}/${ircChannel}`;
...
Provisioner.prototype._leaveIfUnprovisioned = Promise.coroutine(function* (req, roomId, server, ircChannel) {
    try {
        yield Promise.all([
...
// unlinked.
Provisioner.prototype._partUnlinkedIrcClients = Promise.coroutine(function* (req, roomId, server, ircChannel) {
    // Get the full set of room IDs linked to this #channel
    let matrixRooms = yield this._ircBridge.getStore().getMatrixRoomsForChannel(server, ircChannel);
    ...
    req.log.info(`Unlinked user_id tallies for ${ircChannel}: ${JSON.stringify(joinedUserCounts)}`);
});
Provisioner.prototype._leaveMatrixVirtuals = Promise.coroutine(function* (req, roomId, server, ircChannel) {
    const asBot = this._ircBridge.getAppServiceBridge().getBot();
    const roomChannels = yield this._ircBridge.getStore().getIrcChannelsForRoomId(roomId);
    ...
    this._ircBridge.memberListSyncers[server.domain].addToLeavePool(roomInfo.remoteJoinedUsers, roomId, ircChannel);
});
// Cause the bot to leave the matrix room if there are no other channels being mapped to
// this room
Provisioner.prototype._leaveMatrixRoomIfUnprovisioned = Promise.coroutine(function* (req, roomId) {
