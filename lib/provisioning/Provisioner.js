...
//  that might be rate-limited. If rate limiting occurs, err.data.retry_after_ms
//  will be set to the number of MS to wait before resending.
//
...
//  after [fn]. If an exception is raised and appears to have been due to rate
//  limiting, another attempt will occur after the suggested retry delay + 5s.
//
//  If an error occurs for any other reason, an error is logged.
//
//  If the number of attempts is reached, the returned promise rejects.
function retry(attempts, obj, fn) {
    let defered = promiseutil.defer();

    // Remove first two args
    var args = Array.from(arguments).slice(3);

    let tryAgain = Promise.coroutine(function*() {
        try {
            ...
            log.error(`Error doing rate limited action (${msg})`);

            if (err.data && err.data.retry_after_ms && attempts > 0) {
                attempts = attempts - 1;
                if (attempts === 0) {
                    defered.reject(new Error(`Too many attempts to do rate limited action`));
                 }

                log.info(`Trying to do rate limited action, ${attempts} attempts remaining.`);
                setTimeout(tryAgain, err.data.retry_after_ms + 5000);
            }
        }
    });
...
Provisioner.prototype._userHasProvisioningPower = Promise.coroutine(
    function*(userId, roomId) {
        ...
        yield retry(100, matrixClient, matrixClient.joinRoom, roomId).timeout(600000);
