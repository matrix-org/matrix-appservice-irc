...
DebugApi.prototype.sendIRCCommand = function (server, user, body) {
    ...
    let client = this._getClient(server, user);
    if (!client) {
        return Promise.resolve("User " + user + " does not have a client on " + server.domain + "\n");
    }
    if (!client.unsafeClient) {
        return Promise.resolve("There is no underlying client instance.\n");
    }
...
DebugApi.prototype.killPortal = Promise.coroutine(function* (req, response) {
    ...
    log.warn(`Requested deletion of portal room alias ${roomId} through debug API
Domain: ${domain}
Channel: ${channel}
Leave Notice: ${notice}
Remove Alias: ${remove_alias}`);
    // Find room
    let room = yield this.ircBridge.getStore().getRoom(roomId, domain, channel, "alias");
    if (room === null) {
    ...
    this.ircBridge.getStore().removeRoom(roomId, domain, channel, "alias");
    ...
    try {
        yield this.ircBridge.getProvisioner()._leaveIfUnprovisioned({ log: log }, roomId, server, channel);
    }
    catch (e) {
