...
module.exports.hooks = {
    matrix: {
        onInvite: function(event) {
            var rlog = logging.newRequestLogger(log);
            ...
            rlog.info("onInvite: %s", JSON.stringify(event));
            ...
            var sucFn = function(err) {
                rlog.info("onInvite: Processed invite from %s to room %s",
            ...
            function(ircUser) {
                ...
                if (isPmRoom) {
                    ...
                    rlog.error("This room isn't a 1:1 chat!");
        ...
        onMessage: function(event, rlog) {
            if (!rlog) {
                rlog = logging.newRequestLogger(log);
            }
            rlog.info("[M->I]%s usr=%s rm=%s", event.type, event.user_id, 
                event.room_id);

            if (processingInvitesForRooms[event.room_id]) {
                rlog.info("Holding message until invite for room %s is done.",
                    event.room_id);
                holdEvent(event, rlog);
                return;
            }
            ...
            function(ircRooms) {
                if (ircRooms.length == 0) {
                    rlog.info("No mapped channels.");
                    return;
                }
                ircRooms.forEach(function(ircRoom) {
                    rlog.info("Relaying message in %s on %s", 
                    ...
                    ircLib.getVirtualIrcUser(ircRoom.server, event.user_id).then(
                        function(ircUser) {
                            if (!ircUser) {
                                rlog.error("Unknown IRC user for user ID %s", 
                                    event.user_id);
                                return;
                            }
                            rlog.info("Obtained virtual IRC user: %s", 
                                ircUser.nick);
                            ircUser.sendAction(ircRoom, ircAction);
                    }).done(function() {
                        rlog.info("[M->I] Sent message.");
                    }, function(err) {
                        rlog.error("[M->I]Failed to relay Matrix message: %s", 
                            JSON.stringify(err));
                    });
                });
            }, function(err) {
                rlog.error("Failed to get rooms for room ID %s", 
                    event.room_id);
            });
        },
        onAliasQuery: function(roomAlias) {
            var rlog = logging.newRequestLogger(log);
            rlog.info("onAliasQuery %s", roomAlias);
            var defer = q.defer();
            ...
            if (!channelInfo.channel) {
                rlog.info("Unknown alias: %s", roomAlias);
                return q.reject();  // bad alias
            }
            if (!channelInfo.server.shouldMapAllRooms()) {
                rlog.error("This server does not allow alias mappings.");
                return q.reject();
            }
        ...
        onUserQuery: function(userId) {
            var rlog = logging.newRequestLogger(log);
            rlog.info("onUserQuery: %s", userId);
    ...
    irc: {
        onMessage: function(server, from, to, action) {
            var rlog = logging.newRequestLogger(log);
            rlog.info("[I->M]onMessage: from=%s to=%s action=%s",
            ...
            if (!mxAction) {
                rlog.error("Couldn't map IRC action to matrix action");
                return;
            }

            var errFn = function(err) {
                rlog.error("[I->M]Failed to relay IRC message: %s", 
                    JSON.stringify(err));
            };
            ...
            protocols.mapToMatrix("users", fromUser).then(function(user) {
                virtualMatrixUser = user;
                rlog.info("Mapped real IRC user to %s", JSON.stringify(user));
                if (virtualIrcUser) {
                    // this is actually a PM
                    if (!server.allowsPms()) {
                        rlog.error("Server %s disallows PMs.", server.domain);
                        return;
                    }
                    ...
                    store.getPmRoom(
                        ...
                        if (room) {
                            rlog.info("[I->M]Relaying PM in room %s", room.roomId);
                        ...
                        rlog.info("Creating a PM room with %s", 
                    ...
                    store.getRoomsForChannel(server, to).then(function(matrixRooms) {
                        matrixRooms.forEach(function(room) {
                            rlog.info("[I->M]Relaying in room %s", room.roomId);
                            matrixLib.sendAction(
                                room, virtualMatrixUser, mxAction
                            ).done(undefined, errFn);
                        });
