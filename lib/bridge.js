...
module.exports.hooks = {
    matrix: {
        onInvite: function(event) {
            ...
            // TODO: AS bot should join any 1:1 room and mark it as the admin
            // room for that user_id
            ...
            if (userInvitedBot) {
                ...
                store.getAllKnownRoomIds().then(function(roomIds) {
                    if (roomIds.indexOf(event.room_id) !== -1) {
                        // known room
                        return mxReq.joinRoom(event.room_id, undefined);
                    }
                    return q.reject(
            ...
            ircLib.matrixToIrcUser(invitedUser).then(function(ircUser) {
                ...
                return mxReq.joinRoom(event.room_id, virtualMatrixUser);
            }).then(function() {
                req.log.info(
        ...
        onMessage: function(event, existingRequest) {
            ...
            // TODO: If this is a message in an AS admin room and the msg is to 
            // join a channel, make sure they are authed and then send an invite
