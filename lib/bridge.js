...
// === Admin room handling ===
var onAdminMessage = function(event, req, room) {
    var mxReq = matrixLib.getMatrixLibFor(req);
    ...
    if (event.content.body.indexOf("!join ") === 0) {
        ...
        function(user) {
            ...
            if (user) {
                ...
                // If the mapping doesn't exist: track the server/chan and store
                ...
                // get the room corresponding to
                // send the invite out
                mxReq.invite(room, event.user_id).done(req.sucFn, req.errFn); */
            }
            else {
                var notice = actions.matrix.createNotice(
