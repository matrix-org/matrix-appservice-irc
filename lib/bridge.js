...
module.exports.hooks = {
    matrix: {
        onInvite: function(event) {
            ...
            if (!virtualIrcUserTarget.nick) {
                console.error("Invite isn't for an IRC user.");
                return;
            }
            ...
            createMatrixUserWithIrcUserId(userId).then(function(user) {
                ...
                if (isPmRoom) {
                    ...
                    console.error("This room isn't a 1:1 chat!");
                ...
                console.log("onInvite: Processed invite.");
            }, function(err) {
        ...
        onMessage: function(event) {
            console.log("[M->I]%s usr=%s rm=%s", event.type, event.user_id, 
                event.room_id);

            var msg;
            if (event.content.format === "org.matrix.custom.html") {
                msg = formatting.htmlToIrc(event.content.formatted_body);
            }
            if (!msg) {
            ...
            store.getRoomsForRoomId(event.room_id).done(function(ircRooms) {
                    ...
                    ircLib.getVirtualIrcUser(ircRoom.server, event.user_id).then(
                        function(ircUser) {
                            ...
                            if (event.type === "m.room.message") {
                                ...
                                var topic = event.content.topic;
                                return ircUser.setTopic(ircRoom, topic);
                            }
                    }).done(function() {
                ...
                console.error("Failed to get rooms for room ID %s", 
                    event.room_id);
            });
        },
        onAliasQuery: function(roomAlias) {
            ...
            if (!channelInfo.server.shouldMapAllRooms()) {
                console.error("This server does not allow alias mappings.");
                return q.reject();
            }
    ...
    irc: {
        onMessage: function(server, from, to, kind, msg) {
            ...
            var htmlMsg = formatting.ircToHtml(msg);
            var isHtml = htmlMsg !== msg;
            ...
            store.getRoomsForChannel(server, to).done(function(matrixRooms) {
                if (matrixRooms.length == 0) {
                    // target is a PM and we allow them to be passed to Matrix
                    if (to.indexOf("#") !== 0 && server.allowsPms()) {
                        var virtualIrcUser = ircLib.getVirtualUserByNick(
                            server, to
                        );
                        if (!virtualIrcUser) {
                        ...
                        matrixLib.getMatrixUser(server.userPrefix+from).then(
                            ...
                            if (isHtml) {
                                return matrixLib.sendHtml(
                                    roomForInitiator, fromUser, msgTypes[kind],
                                    htmlMsg
                                );
                            }
                            else {
                ...
                matrixLib.getMatrixUser(server.userPrefix+from).done(function(user) {
                    matrixRooms.forEach(function(room) {
                        ...
                        if (kind === "topic") {
                            return matrixLib.setTopic(
                                room, user, msg
                            )
                        }
                        else if (isHtml) {
                            matrixLib.sendHtml(
                                room, user, msgTypes[kind], htmlMsg
                            ).done(undefined, errFn);
                        }
                        else {
