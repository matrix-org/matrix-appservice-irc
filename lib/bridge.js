...
// TODO: pool based on room_id?
// TODO: Timer this if >10sec then log?
var newRequest = function() {
    ...
    request.defer.promise.done(function() {
        ...
        var delta = Date.now() - request.start;
        request.log.debug("FAILED - %s ms", delta);
    });
    return request;
};
...
module.exports.hooks = {
    matrix: {
        ...
        onMessage: function(event, existingRequest) {
            var req = existingRequest || newRequest();
            ...
            function(ircRooms) {
                ...
                ircRooms.forEach(function(ircRoom) {
                    ...
                    ircLib.getVirtualIrcUser(ircRoom.server, event.user_id).then(
                        ...
                        req.log.info("[M->I] Sent message.");
                    }, function(err) {
                        req.log.error("[M->I]Failed to relay Matrix message: %s", 
                            JSON.stringify(err));
                    });
                });
            }, function(err) {
