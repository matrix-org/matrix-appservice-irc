    ...
    matrix: {
        ...
        onJoin: function(event, user) {
            ...
            store.getIrcChannelsForRoomId(event.room_id).done(function(ircRooms) {
                ...
                ircLib.matrixToIrcUser(user).then(function(ircUser) {
                    return req.ircLib.getVirtualIrcUser(
                        ircUser.server, user.userId
                    );
                }).done(function(virtualIrcUser) {
                    // join each channel
                    ircRooms.forEach(function(room) {
                        promises.push(virtualIrcUser.joinChannel(room.channel));
                    });
                    q.all(promises).done(req.sucFn, req.errFn);
                }, req.errFn);
            }, req.errFn);
