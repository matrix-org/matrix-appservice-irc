...
var onAdminMessage = function(event, req, adminRoom) {
    ...
    if (event.content.body.indexOf("!join ") === 0) {
        // Format is: "!join irc.example.com #channel"
        var segments = event.content.body.split(" ");
        var privServers = ircLib.getPrivateServers();
        var ircServer = null;
        for (var i=0; i<privServers.length; i++) {
            if (privServers[i].domain === segments[1]) {
                ircServer = privServers[i];
                break;
            }
        }
        var ircChannel = segments[2];
        if (!ircServer || !ircChannel || ircChannel.indexOf("#") !== 0) {
            var networkList = "";
            for (var i=0; i<privServers.length; i++) {
                networkList += privServers[i].domain+"\n";
            }
            var notice = actions.matrix.createNotice(
                "Format: '!join example.com #channelname'\n"+
                "Valid IRC networks:\n"+networkList
            );
        ...
        // if they are authed to join, then send an invite out.
        store.getAuthenticatedUserById(event.user_id, ircServer.domain).done(
        function(user) {
            var authUrl = ircServer.clientAuthUrl;
            if (!authUrl) {
                req.errFn("IRC server "+ircServer.domain+
                    " proclaims to require auth but no auth url");
                return;
            }

            if (user) {
                // Check if this mapping already exists: if it does then just
                // invite the user to the existing rooms.
                store.getMatrixRoomsForChannel(ircServer, ircChannel).done(
                function(matrixRooms) {
                    if (matrixRooms.length > 0) {
                        var promises = [];
                        matrixRooms.forEach(function(room) {
                            req.log.info(
                                "Inviting %s to room %s", event.user_id, 
                                room.roomId
                            );
                            promises.push(mxReq.invite(room, event.user_id));
                        });
                        q.all(promises).done(req.sucFn, req.errFn);
                        return;
                    }
                    // XXX dupes with onAliasQuery a lot
                    // The mapping doesn't exist: track the server/chan and store
                    // the mapping, and then create a room (invite only) and invite
                    // the user to it.
                    var ircRoom = null;
                    ircLib.trackChannel(ircServer, ircChannel).then(
                    function(room) {
                        ircRoom = room;
                        // implied private
                        return mxReq.createRoomWithUser(
                            undefined, event.user_id, ircChannel
                        );
                    }).then(function(mxRoom) {
                        // store the mapping and return OK
                        store.storeRoomMapping(ircRoom, mxRoom);
                        req.sucFn();
                ...
                var notice = actions.matrix.createNotice(
                    "You need to authenticate to join channels on "+
                    ircServer.domain+" : Login at "+authUrl
                );
                mxReq.sendAction(room, botUser, notice).done(req.sucFn, req.errFn);
            }
        });
    }
    else {
