...
var onAdminMessage = function(event, req, adminRoom) {
    ...
    if (event.content.body.indexOf("!nick") === 0) {
        ...
        // change the nick
        req.ircLib.getVirtualIrcUser(ircServer, event.user_id).then(
        function(ircUser) {
...
module.exports.hooks = {
    matrix: {
        ...
        onJoin: function(event, user) {
            ...
            store.getIrcChannelsForRoomId(event.room_id).done(function(ircRooms) {
                ...
                ircRooms = ircRooms.filter(function(room) {
                    return room.server.shouldMirrorMatrixJoinParts();
                });
                ...
                ircRooms.forEach(function(room) {
                    ...
                    promises.push(
                        req.ircLib.getVirtualIrcUser(room.server, user.userId).then(
                        function(virtualIrcUser) {
                            // join each channel
                            return virtualIrcUser.joinChannel(room.channel);
                        })
                    );
                });
                q.all(promises).done(req.sucFn, req.errFn);
        ...
        onLeave: function(event, user) {
            ...
            var clientList = ircLib.getVirtualIrcUsersForUserId(user.userId);
            // filter out servers which don't mirror matrix join parts
            clientList = clientList.filter(function(client) {
                req.log.debug(
                    "clientConn[%s][%s] mirroring=%s",
                    client.server.domain, client.nick,
                    client.server.shouldMirrorMatrixJoinParts()
                );
                return client.server.shouldMirrorMatrixJoinParts() &&
                    !client.server.claimsUserId(user.userId); // not a virtual user
            });

            var serverLookup = {};
            clientList.forEach(function(virtualIrcUser) {
                serverLookup[virtualIrcUser.server.domain] = virtualIrcUser;
            });
            ...
            store.getIrcChannelsForRoomId(event.room_id).done(function(ircRooms) {
                ...
                ircRooms.forEach(function(ircRoom) {
                    if (ircRoom.server.isMembershipListsEnabled()) {
                        promises.push(membershiplists.checkBotPartRoom(ircRoom));
                    }
                });
                ...
                ircRooms.forEach(function(ircRoom) {
                    ...
                    if (!ircLib.getIrcUserFromCache(ircRoom.server, event.user_id)) {
                        promises.push(req.mxLib.getDisplayName(
                            ...
                            return req.ircLib.getVirtualIrcUser(
                        ...
                        promises.push(req.ircLib.getVirtualIrcUser(
                            ircRoom.server, event.user_id
                        ).then(function(ircUser) {
                            return req.ircLib.sendAction(
                                ircRoom, ircUser, ircAction
        ...
        onUserQuery: function(userId) {
            ...
            var matrixUser = new MatrixUser(userId, true);
    ...
    irc: {
        ...
        onJoin: function(server, nick, chan, kind) {
            if (!server.shouldMirrorIrcJoinParts()) {
