...
module.exports.hooks = {
        ...
        onInvite: function(event) {
            ...
            return protocols.map("users",
                ...
                if (isPmRoom) {
                    ...
                    room.server = realIrcUser.server;
                    room.channel = realIrcUser.nick;
    ...
    irc: {
        onMessage: function(server, from, to, action) {
            ...
            var virtualMatrixUser = undefined;
            ...
            protocols.map("users",
                ...
                log.info("Mapped IRC user to %s", JSON.stringify(user));
                return store.getRoomsForChannel(server, to);
            }).then(function(matrixRooms) {
                log.info("Found %s rooms to send to.", matrixRooms.length);
                // no valid rooms. Make a PM room if it was directed to a user.
                if (matrixRooms.length == 0 && toUser && server.allowsPms()) {
                    // extract the target user ID from the connected virtual
                    // IRC client
                    ...
                    log.info("Creating a PM room with %s", virtualIrcUser.userId);
