...
module.exports.setTopic = function(room, from, topic) {
    ...
    client.setRoomTopic(room.roomId, topic).then(function(suc) {
        console.log("setTopic: %s", JSON.stringify(suc));
        defer.resolve(suc);
    },
    function(err) {
        // XXX should really be trying to join like on sendMessage.
        console.error("setTopic: %s", JSON.stringify(err));
        defer.reject(err);
    });
    return defer.promise;
};
...
module.exports.sendHtml = function(room, from, msgtype, html) {
    msgtype = msgtype || "m.text";
    ...
    function(err) {
        if (err.errcode == "M_FORBIDDEN" && !doNotJoin) {
            ...
            client.joinRoom(room.roomId).done(function(response) {
                ...
                console.error("sendMessageEvent: Couldn't join room: %s",
                    JSON.stringify(err));
                defer.reject(err);
            });
        }
        else {
            console.error("sendMessageEvent: %s", JSON.stringify(err));
            defer.reject(err);
        }
    });
    return defer.promise;
