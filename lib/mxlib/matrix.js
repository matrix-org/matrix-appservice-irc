...
module.exports.joinMappedRooms = function() {
    store.getRoomIdConfigs().done(function(roomIds) {
        ...
        roomIds.forEach(function(roomId) {
            lib.joinRoom(roomId).catch(function(e) {
                log.error(
                    "Failed to join room %s : %s", roomId, JSON.stringify(e)
                );
            });
        });
    });
...
MatrixLib.prototype.addAlias = function(roomId, alias) {
    ...
    var path = "/directory/room/" + encodeURIComponent(alias);
    var body = {
        room_id: roomId
    };
    client._doAuthedRequest(
