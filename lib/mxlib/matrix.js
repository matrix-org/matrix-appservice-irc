...
var baseClientConfig = undefined; // home server url, access token, etc
var clients = {
    // user_id: Client
};
...
var getClientAs = function(userId) {
    ...
    clients[userId] = sdk.cs.createClient(extend({
        userId: userId
    }, baseClientConfig));
    return clients[userId];
};
...
var sendMessageEvent = function(room, from, content, joinState, existingDefer) {
    ...
    function(err) {
        if (err.errcode == "M_FORBIDDEN" && !joinState) {
            ...
            client.joinRoom(room.roomId).done(function(response) {
                sendMessageEvent(room, from, content, "join", defer);
            }, function(err) {
                if (err.errcode == "M_FORBIDDEN" && !joinState) {
...
var getMatrixUser = function(userLocalpart, displayName) {
    ...
    client.register("m.login.application_service", {
        ...
        var userClient = getClientAs(response.user_id);
...
module.exports.joinRoom = function(roomId, matrixUser) {
    ...
    // specified.
    var client = matrixUser ? getClientAs(matrixUser.userId) : getClientAs();
...
module.exports.setMatrixClientConfig = function(config) {
    ...
    clients["bot"] = sdk.cs.createClient(baseClientConfig);
};
