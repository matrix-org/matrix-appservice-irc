...
var collection = {
    ...
    auth_sessions: { db:null, loc:"/auth_sessions.db", defer:q.defer() },
};
...
module.exports.getSession = function(userId, network) {
    var d = q.defer();
    select(getCollection("auth_sessions"), d, {
        user_id: userId,
        network: network
    }, false, function(doc) {
        if (!doc) {
        ...
        var session = new Session(doc.user_id, serverMappings[doc.network], opts);
        return session;
    });
    return d.promise;
};

module.exports.getSessionByToken = function(sessionToken) {
    var d = q.defer();
    select(getCollection("auth_sessions"), d, {
        token: sessionToken
    }, false, function(doc) {
...
module.exports.storeSession = function(session) {
    if (!session) {
        return q.reject("storeSession: Missing session");
    }
    var d = q.defer();
    log.info("storeSession: %s", JSON.stringify(session));
    upsert(getCollection("auth_sessions"), d, {
        user_id: session.userId,
        network: session.server.domain
    },
    {
        $set: {
            user_id: session.userId,
