...
module.exports.connectToDatabase = function(databaseUri) {
    ...
    var promises = [
        dRooms.promise, dConfig.promise
    ];

    roomsDb = new Datastore({
        filename: baseDbName + "/rooms.db",
        autoload: true,
        onload: function(err) {
            if (err) { dRooms.reject(err); } else { dRooms.resolve(); }
        }
    });
    configDb = new Datastore({
        ...
        autoload: true,
        onload: function(err) {
            if (err) { dConfig.reject(err); } else { dConfig.resolve(); }
        }
    });
...
module.exports.getAllKnownRoomIds = function() {
    var d = q.defer();
    select(roomsDb, d, {}, true, function(docs) {
        var roomIds = [];
        for (var i=0; i<docs.length; i++) {
            if (docs[i].room_id) {
...
module.exports.getRoomIdConfigs = function() {
    var d = q.defer();
    select(roomsDb, d, { from_config: true }, true, 
    function(docs) {
        var roomIds = [];
        for (var i=0; i<docs.length; i++) {
            if (docs[i].room_id) {
...
module.exports.removeConfigMappings = function() {
    ...
    del(roomsDb, d, { from_config: true });
    return d.promise;
};
