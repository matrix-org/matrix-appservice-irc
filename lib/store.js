...
var collection = {
    ...
    auth_users: { db:null, loc:"/auth_users.db", defer:q.defer() },
};
...
module.exports.getAuthenticatedUserById = function(userId, network) {
    var d = q.defer();
    select(getCollection("auth_users"), d, {
        user_id: userId,
        network: network
    }, false);
    return d.promise;
...
module.exports.storeAuthenticatedUser = function(authUser) {
    if (authUser.protocol !== "matrix" || !authUser.auth) {
        return q.reject(
            "storeAuthenticatedUser: Invalid user "+JSON.stringify(authUser)
        );
    }
    var d = q.defer();
    log.info("storeAuthenticatedUser: %s", JSON.stringify(authUser));
    upsert(getCollection("auth_users"), d, {
