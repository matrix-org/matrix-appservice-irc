...
"use-strict";
var q = require("q");
var Room = require("./models/rooms").Room;
...
module.exports.storePmRoom = function(room, userId, virtualUserId) {
    var d = q.defer();
    var addr = room.server ? room.server.domain : undefined;

    log.info("storePmRoom (id=%s, addr=%s chan=%s real=%s virt=%s)", 
        room.roomId, addr, room.channel, userId, virtualUserId);
...
module.exports.getPmRoom = function(realUserId, virtualUserId) {
    // sort them so the 'a' and 'b' user IDs match for the store method.
    var d = q.defer();
    select(db.collection("rooms"), d, {
        ...
        room.virtualUserId = doc.virtual_user_id;
        return room;
    })
    return d.promise;
};
