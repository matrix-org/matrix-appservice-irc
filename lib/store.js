...
var insert = function(col, d, objects) {
    col.insert(objects, function(err, result) {
        callbackFn(d, err, result);
    });
};
var upsert = function(col, d, query, updateVals) {
    col.update(query, updateVals, {upsert: true}, function(err, result) {
        callbackFn(d, err, result);
    });
};
var update = function(col, d, query, updateVals) {
    col.update(query, updateVals, {upsert: false}, function(err, result) {
        callbackFn(d, err, result);
    });
};
var del = function(col, d, query) {
    col.remove(query, {multi: true}, function(err, result) {
...
module.exports.setServerFromConfig = function(server, serverConfig) {
    ...
    for (var i = 0; i < channels.length; i++) {
        ...
        for (var k = 0; k < serverConfig.mappings[channel].length; k++) {
            ...
            module.exports.rooms.set(ircRoom, mxRoom, true);
        }
    }
};
