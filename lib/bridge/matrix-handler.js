...
var MatrixAction = require("../models/actions").MatrixAction;
var IrcAction = require("../models/actions").IrcAction;
...
MatrixHandler.prototype._handleInviteFromBot = Promise.coroutine(function*(req, event,
    ...
    yield this.ircBridge.bridge.getIntent(mxUser.userId).join(event.room_id);
});
...
MatrixHandler.prototype._handleInviteFromUser = Promise.coroutine(function*(req, event,
    ...
    yield this.ircBridge.bridge.getIntent(invitedUser.userId).join(event.room_id);
    ...
    yield this.ircBridge.bridge.getIntent(invitedUser.userId).leave(event.room_id);
});
...
MatrixHandler.prototype._onJoin = Promise.coroutine(function*(req, event, user) {
    ...
    ircRooms.forEach(function(room) {
        ...
        promises.push(Promise.coroutine(function*() {
            let bridgedClient = yield self.ircBridge.getBridgedClient(room.server, user.userId);
...
MatrixHandler.prototype._onLeave = Promise.coroutine(function*(req, event, user) {
    ...
    let clientList = this.ircBridge.getBridgedClientsForUserId(user.userId);
