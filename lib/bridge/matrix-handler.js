...
MatrixHandler.prototype._handleInviteFromUser = Promise.coroutine(function*(req, event,
    ...
    let intent = this.ircBridge.bridge.getIntent(invitedUser.userId);
...
MatrixHandler.prototype._onAdminMessage = Promise.coroutine(function*(req, event, adminRoom) {
    ...
    let botUser = new MatrixUser(
        this.ircBridge.getAppServiceUserId(), null, false
    );
...
MatrixHandler.prototype._onInvite = Promise.coroutine(function*(req, event, inviter, invitee) {
    ...
    req.getPromise().finally(() => {
        this._processingInvitesForRooms[event.room_id + event.state_key] = undefined;
    });
...
MatrixHandler.prototype._onJoin = Promise.coroutine(function*(req, event, user) {
    ...
    if (this.ircBridge.getAppServiceUserId() === user.userId) {
...
MatrixHandler.prototype._onAliasQuery = Promise.coroutine(function*(req, roomAlias) {
    ...
    if (matrixRooms.length === 0) {
        ...
        try { // make the matrix room
            ...
            let shouldSetJoinRule = true;
            if (channelInfo.server.shouldPublishRooms()) {
                visibility = "public";
                if (channelInfo.server.getJoinRule() === "public") {
                    shouldSetJoinRule = false; // visibility=public also sets join_rule=public
                }
            }
            else {
                visibility = "private";
                if (channelInfo.server.getJoinRule() === "invite") {
                    shouldSetJoinRule = false; // visibility=private also sets join_rule=invite
                }
            }
            ...
            if (shouldSetJoinRule) {
                yield botIntent.sendStateEvent(newRoomId, "m.room.join_rules", "", {
                    join_rule: channelInfo.server.getJoinRule()
                });
            }

            yield botIntent.sendStateEvent(newRoomId, "m.room.history_visibility", "", {
