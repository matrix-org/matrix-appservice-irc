...
IrcBridge.prototype.sendMatrixAction = Promise.coroutine(function*(room, from, action, req) {
    ...
    if (msgtype) {
        if (action.htmlText) {
            return intent.sendMessage(room.roomId, {
                msgtype: msgtype,
                body: (
        ...
        return intent.setRoomTopic(room.roomId, action.text);
    }
...
IrcBridge.prototype.matrixToIrcUser = function(user) {
    ...
    var servers = this.getServers();
    for (var i = 0; i < servers.length; i++) {
        if (servers[i].claimsUserId(user.userId)) {
            server = servers[i];
            break;
        }
    }
    var ircInfo = {
        server: server,
        nick: server ? server.getNickFromUserId(user.userId) : null
    };
...
IrcBridge.prototype._loginToServer = Promise.coroutine(function*(server) {
    ...
    if (server.shouldJoinChannelsIfNoUsers()) {
        ...
        chansToJoin = this.memberListSyncers[server.domain].getChannelsToJoin();
    }
...
IrcBridge.prototype.getBridgedClient = Promise.coroutine(function*(server, userId,
    ...
    if (storedIrcUser) {
        log.debug("Configuring IRC user from store => %s", storedIrcUser);
