...
var names = require("../irc/names.js");
var IrcServer = require("../irc/server.js").IrcServer;
...
var pool = require("../irc/client-pool");
...
IrcBridge.prototype.run = Promise.coroutine(function*(port) {
    ...
    yield store.connectToDatabase(this.config.ircService.databaseUri); // REMOVE TODO
    ...
    this.bridge = new Bridge({
        ...
        controller: this,
        roomStore: dirPath + "/_rooms.db",
        userStore: dirPath + "/_users.db",
    ...
    yield this.bridge.loadDatabases();

    this.dataStore = new DataStore(this.bridge.getUserStore(), this.bridge.getRoomStore());
    yield this.dataStore.removeConfigMappings();
    ...
    for (var i = 0; i < serverDomains.length; i++) {
        ...
        yield this.dataStore.setServerFromConfig(server, this.config.ircService.servers[domain]);
...
IrcBridge.prototype.sendMatrixAction = Promise.coroutine(function*(room, from, action, req) {
    if (req) {
        req.log.info("sendAction -> %s", JSON.stringify(action));
    }
...
IrcBridge.prototype.matrixToIrcUser = function(user) {
    ...
    if (!ircInfo.server || !ircInfo.nick) {
        return Promise.reject("User ID " + user.getId() + " doesn't map to a server/nick");
    }
...
IrcBridge.prototype.connectToIrcNetworks = function() {
    return store.waitForDatabase().then(() => { // FIXME REMOVE TODO
...
IrcBridge.prototype._loginToServer = Promise.coroutine(function*(server) {
    ...
    if (!bridgedClient) {
        ...
        bridgedClient = pool.createIrcClient(ircUser, null, true, this.bridge);
        log.debug(
    ...
    try {
        yield bridgedClient.connect(this.ircHooks);
    }
    catch (err) {
...
IrcBridge.prototype.getBridgedClient = Promise.coroutine(function*(server, userId,
    ...
    bridgedClient = pool.createIrcClient(ircUser, mxUser, false, this.bridge);

    // check the database for stored config information for this irc client
    // including username, custom nick, nickserv password, etc.
    let storedIrcUser = yield store.ircClients.get(userId, server.domain);
    if (storedIrcUser) {
        log.debug("Configuring IRC user from store => " + storedIrcUser);
    ...
    try {
        yield bridgedClient.connect(this.ircHooks);
        if (!storedIrcUser) {
            yield store.ircClients.set(userId, bridgedClient.ircUser);
        }
        return bridgedClient;
    }
    catch (err) {
...
IrcBridge.prototype.getBotClient = function(server) {
    ...
    return this._loginToServer(server).then(function() {
        return pool.getBot(server);
    });
}
