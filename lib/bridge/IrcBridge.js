...
function IrcBridge(config, registration) {
    ...
    this.joinedRoomList = [];
    const connectionReapingEnabled = config.ircService.connectionReaper &&
        config.ircService.connectionReaper.enabled;
    this.activityTracker = connectionReapingEnabled ? new MatrixActivityTracker(
...
IrcBridge.prototype.run = Promise.coroutine(function*(port) {
    ...
    if (this.activityTracker) {
        ...
        if (!this.config.ircService.connectionReaper.maxIdleTime) {
            this.config.ircService.connectionReaper.maxIdleTime = THIRTY_DAYS;
        }
        let recurseFunction;
        const delay = Math.min(this.config.ircService.connectionReaper.maxIdleTime, MAX_DELAY);
        let lastCall = Date.now();
        // setInterval/setTimeout only works with 32bit integer delays, so working
        // in our own recursive calling function.
        recurseFunction = () => {
            const delayTime = Date.now() - lastCall;
            lastCall = Date.now();
            let nextDelay = Math.min(
                this.config.ircService.connectionReaper.maxIdleTime - delayTime,
                MAX_DELAY
            );
            setTimeout(
                () => {
                    this._connectionReap().finally(() => {
                        recurseFunction();
                    });
                },
                nextDelay
...
IrcBridge.prototype._connectionReap = Promise.coroutine(function*() {
    const maxIdleTime = this.config.ircService.connectionReaper.maxIdleTime;
    const req = new BridgeRequest(this._bridge.getRequestFactory().newRequest());
    const reason = this.config.ircService.connectionReaper.reasonMsg || "User is inactive";
    yield Promise.all(Object.keys(this.memberListSyncers).map(async (server) => {
        log.info(`Connection reaping for ${server}`);
        const rooms = await this.memberListSyncers[server].getSyncableRooms(true);
        const users = [];
        ...
        log.info(`Found ${users.length} real users for ${server}`);
        let offlineCount = 0;
        for (const user of users) {
            const status = await this.activityTracker.isUserOnline(user, maxIdleTime);
            if (!status.online) {
                const clients = this.pool.getBridgedClientsForUserId(userId);
                await this.ircBridge.matrixHandler.quitUser(req, userId, clients, null, reason);
                offlineCount++;
            }
        }
        log.info(`Quit ${offlineCount} *offline* real users for ${server}.`);
    }));
});
