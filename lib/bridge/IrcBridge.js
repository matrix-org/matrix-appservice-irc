...
var IdentGenerator = require("../irc/names.js");
...
IrcBridge.prototype.getIdentGenerator = function() {
    return this.identGenerator;
};
...
IrcBridge.prototype._loginToServer = Promise.coroutine(function*(server) {
    ...
    if (!bridgedClient) {
        ...
        bridgedClient = pool.createIrcClient(ircUser, null, true, this);
        log.debug(
...
IrcBridge.prototype.getBridgedClient = Promise.coroutine(function*(server, userId,
    ...
    bridgedClient = pool.createIrcClient(ircUser, mxUser, false, this);

    // check the database for stored config information for this irc client
    // including username, custom nick, nickserv password, etc.
    let storedIrcUser = yield this.getStore().getIrcClient(userId, server.domain);
    if (storedIrcUser) {
    ...
    try {
        yield bridgedClient.connect(this.ircHooks);
        if (!storedIrcUser) {
            yield this.getStore().storeIrcClient(userId, bridgedClient.ircUser);
        }
        return bridgedClient;
    }
    catch (err) {
