...
const { Bridge, MatrixUser,
    PrometheusMetrics, Logging,
    ForeignNetworkError } = require("matrix-appservice-bridge");
...
function IrcBridge(config, registration) {
    ...
    const serverListing = Object.values(this.config.ircService.servers);

    const networkName = serverListing.length === 1 ? serverListing[0].name : "irc";
...
IrcBridge.prototype._addRequestCallbacks = function() {
    ...
    this._bridge.getRequestFactory().addDefaultTimeoutCallback((req) => {
        ...
        stats.request(isFromIrc, "fail", req.getDuration());
        this._logMetric(req, "fail");
        req.reject(new ForeignNetworkError(`Event could not be relayed. Died after ${DEAD_TIME_MS}`));
    }, DEAD_TIME_MS);
}
