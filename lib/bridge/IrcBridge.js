...
function IrcBridge(config, registration) {
    ...
    this.ircServers = [];
    this._bridge = null; // matrix-appservice-bridge Bridge
    ...
    this._ircEventBroker = new IrcEventBroker(this._clientPool);
    this.dataStore = null;
...
IrcBridge.prototype.getIrcEventBroker = function() {
    return this._ircEventBroker;
};
...
IrcBridge.prototype.run = Promise.coroutine(function*(port) {
    ...
    this.dataStore = new DataStore(this._bridge.getUserStore(), this._bridge.getRoomStore());
...
IrcBridge.prototype._loginToServer = Promise.coroutine(function*(server) {
    ...
    if (!bridgedClient) {
        ...
        bridgedClient = this._clientPool.createIrcClient(ircUser, null, true, this);
        log.debug(
...
IrcBridge.prototype.getBridgedClient = Promise.coroutine(function*(server, userId,
    ...
    bridgedClient = this._clientPool.createIrcClient(ircUser, mxUser, false, this);
