...
function IrcBridge(config, registration) {
    ...
    this.joinedRoomList = [];
    const connectionReapingEnabled = config.ircService.connectionReaper && config.ircService.connectionReaper.enabled;
...
IrcBridge.prototype.run = Promise.coroutine(function*(port) {
    ...
    if (this.activityTracker) {
        ...
        // Setup connection reaper
        setInterval(
            this._connectionReap.bind(this),
            this.config.connectionReaper.maxIdleTime || THIRTY_DAYS
        );
    }
...
IrcBridge.prototype._connectionReap = Promise.coroutine(function*() {
    const maxIdleTime = config.ircService.connectionReaper.maxIdleTime;
    const req = new BridgeRequest(this.ircBridge._bridge.getRequestFactory().newRequest());
    const reason = config.ircService.connectionReaper.reasonMsg || "User is inactive";
    yield Promise.all(Object.keys(this.memberListSyncers).map(async (server) => {
        ...
        log.info(`Found ${users.size} real users for ${server}`);
        let offlineCount = 0;
