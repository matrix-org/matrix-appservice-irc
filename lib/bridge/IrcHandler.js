...
function IrcHandler(ircBridge) {
    ...
    this._rejoinPromises = {
        // nick: Promise that resolves if the user joins a channel having quit
        //       previously
    };
    this._rejoinResolvers = {
        // nick: function to call when a user joins
    };
}
...
IrcHandler.prototype.onJoin = Promise.coroutine(function*(req, server, joiningUser, chan, kind) {
    ...
    if (this._rejoinPromises[nick]) {
        this._rejoinResolvers[nick]();
    }
...
IrcHandler.prototype.onPart = Promise.coroutine(function*(req, server, leavingUser, chan, kind) {
    ...
    if (kind === "quit") {
        ...
        yield Promise.all(matrixRooms.map((room) => {
            req.log.info(
                "Setting presence of %s in room %s to %s",
                matrixUser.getId(), room.getId(), presence
            );
            return this.ircBridge.getAppServiceBridge().getIntent(
                matrixUser.getId()
        ...
        this._rejoinPromises[nick] = new Promise((resolve, reject) => {
            this._rejoinResolvers[nick] = resolve;
            setTimeout(() => {
        ...
        try {
            yield this._rejoinPromises[nick];
