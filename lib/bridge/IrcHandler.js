...
function IrcHandler(ircBridge) {
    ...
    // Maintain a map of topics so as not to repeatidly send duplicate topics into
    // a room.
    this._roomIdToTopic = {
        // room_id: 'Topic of the room'
    };
}
...
IrcHandler.prototype.onMessage = Promise.coroutine(function*(req, server, fromUser,
    ...
    // For topics, set matrix topic as AS bot user
    if (action.type === "topic") {
        // Only bridge topics for rooms created by the bridge, via !join or an alias
        let origins = ["join", "alias"];
        let mappings = yield this.ircBridge.getStore().getMappingsForChannelByOrigin(
            server, channel, origins
        );
        let promises = mappings.map((entry) => {

            if (this._roomIdToTopic[entry.matrix.getId()] === action.text) {
                req.log.info(
            ...
            req.log.info(
                "Message is a topic, using AS bot to set topic in room %s: %s",
                entry.matrix.getId(),
                action.text
            );
            ...
            return this.ircBridge.getAppServiceBridge().getIntent().setRoomTopic(
                ...
                req.log.info(
                    "Set topic of %s ('%s', now cached)",
                    entry.matrix.getId(),
                    action.text
                );

                this._roomIdToTopic[entry.matrix.getId()] = action.text;
            }, (err) => {
