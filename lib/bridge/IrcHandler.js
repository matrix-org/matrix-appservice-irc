...
function IrcHandler(ircBridge) {
    ...
    // Use a queue to keep the setting of topics in rooms atomic in order to
    // prevent races involving several topics being received from IRC in quick
    // succession. This queue will cause them to block unless the server, channel
    // and topic are the same as an ongoing promise to set a topic, whihc will in
    // that case be returned.
    this.topicQueue = new Queue(this._serviceTopicQueue.bind(this));
}
...
IrcHandler.prototype._serviceTopicQueue = Promise.coroutine(function*(item) {
    ...
    try {
        ...
        item.req.log.info(
            "Set topics: ",
            item.entries.map(
                (entry) => `id = ${entry.matrix.getId()}, topic = '${entry.matrix.topic}'`
            ).join('\n')
        );
    }
    catch (err) {
        req.log.error('Failed to set topic(s) ${err.message}');
    }
});
