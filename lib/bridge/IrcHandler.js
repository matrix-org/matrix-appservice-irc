...
IrcHandler.prototype.onPart = Promise.coroutine(function*(req, server, leavingUser, chan, kind) {
    ...
    let shouldBridgePart = true;
    if (kind === "quit" && server.shouldDebouceParts()) {
        shouldBridgePart = yield this.quitDebouncer.debounceQuit(req, server, matrixUser);
    }
