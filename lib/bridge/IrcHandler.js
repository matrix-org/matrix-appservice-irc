...
var QueuePool = require("../util/QueuePool.js");
...
function IrcHandler(ircBridge) {
    ...
    this._joinQueuePool = new QueuePool(3, this._throttleJoin.bind(this));
}
...
IrcHandler.prototype.onJoin = Promise.coroutine(function*(req, server, joiningUser, chan, kind) {
    ...
    yield this._joinQueuePool.enqueue(chan + " " + kind + " " + nick, {
        server: server,
        chan: chan,
...
IrcHandler.prototype._throttleJoin = Promise.coroutine(function*(item) {
    // get virtual matrix user
    let matrixUser = yield this.ircBridge.getMatrixUser(item.joiningUser);
    let matrixRooms = yield this.ircBridge.getStore().getMatrixRoomsForChannel(
        item.server, item.chan
    );

    let promises = matrixRooms.map((room) => {
        item.req.log.info("Joining room %s and setting presence to online", room.getId());
        return Promise.all([
    ...
    if (matrixRooms.length === 0) {
        item.req.log.info("No mapped matrix rooms for IRC channel %s", item.chan);
    }
    else {
