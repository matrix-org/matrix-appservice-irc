...
IrcHandler.prototype.onMode = Promise.coroutine(function*(req, server, channel, by,
    ...
    if (!bridgedClient) {
        req.log.info("No bridged client for %s", arg);
        return;
    }

    if (!bridgedClient.unsafeClient) {
        req.log.info(`Bridged client for ${arg} has no IRC client.`);
        return;
    }

    const userPrefixes = bridgedClient.unsafeClient.chanData(channel).users[arg];
    console.log(userPrefixes);
    const userPowers = [];
    ...
    req.log.info(
        `Mode ${mode} received for ${arg}, prefixes now = ${userPrefixes} ` +
