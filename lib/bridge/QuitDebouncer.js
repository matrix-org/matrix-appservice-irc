...
QuitDebouncer.prototype.debounceQuit = Promise.coroutine(function*(req, server, matrixUser, nick) {
    ...
    let threshold = 5;// Need 5Hz of quits to call net-split
    this._quitTimesMs[server.domain].push(now);

    // Filter out timestamps from more than 1s ago
    this._quitTimesMs[server.domain] = this._quitTimesMs[server.domain].filter(
        (t) => t > now - windowMs
    );
    ...
    let isSplitOccuring = this._quitTimesMs[server.domain].length > threshold;
