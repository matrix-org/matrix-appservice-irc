...
MemberListSyncer.prototype.sync = Promise.coroutine(function*() {
    ...
    // However, we do not want to do N joins if we know they were already joined according
    // to /joined_members, so cheekily update the Intent backing store to say which
    // users are joined to they no-op when Inject.join is called.
    rooms.forEach((roomInfo) => {
        let bridge = this.ircBridge.getAppServiceBridge();
        // FIXME: Gut-wrenching to the highest degree
        roomInfo.remoteJoinedUsers.forEach((userId) => {
            bridge._setMemberEntry(roomInfo.id, userId, "join");
        });
        roomInfo.realJoinedUsers.forEach((userId) => {
            bridge._setMemberEntry(roomInfo.id, userId, "join");
        });
    });
});
