            ...
            while (kickIntent) {
                try {
                    yield kickIntent.kick(
                        ...
                        `Connection limit has been reached for ${room.server.domain}. Please try again later.`
                    );
                    break;
                } catch (err) {
                    req.log.warn(`User was not kicked. Retrying in ${KICK_RETRY_DELAY_MS}ms. ${err}`);
                    yield Promise.delay(KICK_RETRY_DELAY_MS);
                }
            }
