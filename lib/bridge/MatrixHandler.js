...
MatrixHandler.prototype._onAdminMessage = Promise.coroutine(function*(req, event, adminRoom) {
    ...
    if (cmd === "!nick") {
        ...
        // Get all the rooms
        // Send one message saying "SPAM" every second to all rooms until receive !spam

        this._spamming = !this._spamming;

        if (!this._spamming) {
            clearInterval(this._spamming_interval);
            return;
        }

        let notice = new MatrixAction("notice", `SPAM\n` );

        let rooms = yield this.ircBridge._bridge.getRoomStore().getEntriesByRemoteRoomData(Object.create(null)).map((e) => e.matrix);

        this._spamming_interval = setInterval(() =>
            rooms.forEach((room) => {
                this.ircBridge.sendMatrixAction(room, botUser, notice);
            }), 1000
        );
    }
});
