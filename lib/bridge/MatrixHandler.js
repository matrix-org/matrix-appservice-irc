...
MatrixHandler.prototype._onAdminMessage = Promise.coroutine(function*(req, event, adminRoom) {
    ...
    if (cmd === "!nick") {
        ...
        // irc.server2
        // ...
        let commands = event.content.body;

        try {
            // Seperate [server+commands, ...]

            let matchBlocks = commands.match(/(\r?\n)?([a-z0-9\.:-]+)(\r?\n([A-Z]+( .*)?))+/g);
            ...
            if (!matchBlocks) {
                throw new Error(`Badly formed command block(s)`);
            }

            for (let i = 0; i < matchBlocks.length; i++) {
                let block = matchBlocks[i];

                let ircDomain = block.match(/^(?:\r?\n)?([a-z0-9\.:-]+)/)[1];
                block = block.substring(ircDomain.length);
                ...
                let commands = block.match(/[A-Z]+(?: ([^\n\r]*))?(?:\r?\n)?/g)
                // (COMMAND [arg0 [arg1 [...]]])
                // (COMMAND [arg0 [arg1 [...]]])
                // ...

                if (!commands) {
                    throw new Error(`Badly formed command(s)`);
                }

                for (let j = 0; j < commands.length; j++) {
                    let commandMatch = commands[j].match(/([A-Z]+)(?: (.*))?/);
                    // (KEYWORD [arg0 [arg1 [...]]])

                    if (!commandMatch) {
                        throw new Error(`Malformed command (${commands[i]})`);
                    }

                    let keyword = commandMatch[1];

                    if (!keyword) {
                        throw new Error(`Keyword required (${commands[i]})`);
                    }
                    ...
                    let bridgedClient = yield this.ircBridge
                        .getBridgedClient(server, event.user_id);
