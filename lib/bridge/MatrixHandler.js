...
MatrixHandler.prototype._onAdminMessage = Promise.coroutine(function*(req, event, adminRoom) {
    ...
    if (cmd === "!nick") {
        ...
        let fmt = '(COMMAND [arg0 [arg1 [...]]]) | irc.server';
        req.log.info(`No valid (old form) admin command, will try new format: ${fmt}`);

        // Assumes commands have the form "(COMMAND [arg0 [arg1 [...]]]) | irc.server"
        let command = event.content.body;

        let matches = command.match(/^([a-z0-9\.:-]+)|(?:([A-Z]+)(?: (.*))?)$/);
        try {
            if (!matches) {
                throw new Error(`Badly formed command`);
            }
            let ircDomain = matches[1];
            let keyword = matches[2];
            let argsJoined = matches[3];
            ...
            if (ircDomain) {
                ...
                this._adminCommandServer = server;
            }
            else if (keyword) {
                if (!this._adminCommandServer) {
                    throw new Error('Please provide domain before invoking commands');
                }

                let acceptedKeywords = ['JOIN', 'PART', 'NOTICE', 'TOPIC', 'INVITE'];

                if (acceptedKeywords.indexOf(keyword) == -1) {
                    throw new Error(`Keyword unrecognised (${keyword})\n`+
                        'Accepted keywords: ' + acceptedKeywords.join(', '));
                }

                let sendArgs = argsJoined.split(' ');
                sendArgs.unshift(keyword);

                let bridgedClient = yield this.ircBridge
                    .getBridgedClient(this._adminCommandServer, event.user_id);

                if (!bridgedClient.unsafeClient) {
                    throw new Error('Possibly disconnected');
                }

                Client.prototype.send.apply(bridgedClient.unsafeClient, sendArgs);
            }
            else {
                throw new Error(`Keyword and arguments required`);
            }
        }
        catch (err) {
            let notice = new MatrixAction("notice",
