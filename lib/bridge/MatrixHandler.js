...
MatrixHandler.prototype._onMessage = Promise.coroutine(function*(req, event) {
    ...
    Object.keys(otherMatrixRoomIdsToServers).forEach((roomId) => {
        ...
        promises.push(
            this.ircBridge.sendMatrixAction(
                ...
                // This is non-fatal, so don't cause the whole promise chain to fail.
                req.log.error(`Failed to relay matrix message to ${roomId}`);
            })
        );
    });
    ...
    try {
        ...
        throw new ForeignNetworkError(`Failed to send to ${ircRoom.channel}.`);
    }
});
