 ...
 */
MatrixHandler.prototype._handleAdminRoomInvite = Promise.coroutine(function* (req, event, inviter, botUser) {
    ...
    try {
        ...
        if (plumbedState.status === "enabled") {
            req.log.info('This room is marked for plumbing (m.room.plumbing.status = "enabled"). ' +
 ...
 */
MatrixHandler.prototype._handleInviteFromBot = Promise.coroutine(function* (req, event, invitedIrcUser) {
...
MatrixHandler.prototype._handleInviteFromUser = Promise.coroutine(function* (req, event, invitedIrcUser) {
    ...
    if (!invitedIrcUser.server.allowsPms()) {
        ...
        yield this.ircBridge.sendMatrixAction(mxRoom, invitedUser, new MatrixAction("notice", MSG_PMS_DISABLED), req);
    ...
    if (!invitedIrcUser.server.shouldFederatePMs()) {
        ...
        if (localpart !== this.ircBridge.domain) {
            req.log.error("Accepting invite, and then leaving: This server does not allow federated PMs.");
            yield intent.join(event.room_id);
            yield this.ircBridge.sendMatrixAction(mxRoom, invitedUser, new MatrixAction("notice", MSG_PMS_DISABLED_FEDERATION), req);
            yield intent.leave(event.room_id);
            return;
        }
    ...
    let isPmRoom = (joinedMembers.length === 2 && joinedMembers.indexOf(event.user_id) !== -1);
    if (isPmRoom) {
        // nick is the channel
        let ircRoom = new IrcRoom(invitedIrcUser.server, invitedIrcUser.nick);
        yield this.ircBridge.getStore().setPmRoom(ircRoom, mxRoom, event.user_id, event.state_key);
        return;
    }
    req.log.error("This room isn't a 1:1 chat!");
    // whine that you don't do group chats and leave.
    let notice = new MatrixAction("notice", "Group chat not supported.");
    try {
...
MatrixHandler.prototype._onAdminMessage = Promise.coroutine(function* (req, event, adminRoom) {
    ...
    if (this._memberTracker) {
        ...
        members = this._memberTracker.getState(adminRoom.getId(), 'm.room.member').filter(function (m) {
    ...
    if (members.length > 2) {
        req.log.error(`_onAdminMessage: admin room has ${members.length}` +
            ` users instead of just 2; bot will leave`);
        // Notify users in admin room
        let notice = new MatrixAction("notice", "There are more than 2 users in this admin room");
        yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
        yield this.ircBridge.getAppServiceBridge().getIntent(botUser.getId()).leave(adminRoom.getId());
        return;
    }
    ...
    if (cmd === "!help") {
        ...
        let notice = new MatrixAction("notice", null, `This is an IRC admin room for controlling your IRC connection and sending ` +
    ...
    if (ircServer) {
        ...
        // default to the only server we know about if we only bridge 1 thing.
            ...
            let notice = new MatrixAction("notice", "A server address must be specified.");
            yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
            return;
        }
    }
    if (cmd === "!nick") {
        ...
        if (!ircServer.allowsNickChanges()) {
            let notice = new MatrixAction("notice", "Server " + ircServer.domain + " does not allow nick changes.");
            yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
            return;
        }
        ...
        try {
            ...
            let config = yield this.ircBridge.getStore().getIrcClientConfig(event.user_id, ircServer.domain);
            if (!config) {
                config = IrcClientConfig.newConfig(bridgedClient.matrixUser, ircServer.domain, nick);
            }
        ...
        req.log.info("%s wants to join the channel %s on %s", event.user_id, ircChannel, ircServer.domain);
        ...
        let matrixRooms = yield this.ircBridge.getStore().getMatrixRoomsForChannel(ircServer, ircChannel);
        if (matrixRooms.length === 0) {
            ...
            let response = yield this.ircBridge.getAppServiceBridge().getIntent(event.user_id).createRoom({
                options: {
                    name: ircChannel,
                    visibility: "private",
                    preset: "public_chat",
            ...
            // +s and +i correctly.
            this.ircBridge.publicitySyncer.initModeForChannel(ircServer, ircChannel).catch((err) => {
                log.error(`Could not init mode for channel ${ircChannel} on ${ircServer.domain}`);
            });
            req.log.info("Created a room to track %s on %s and invited %s", ircRoom.channel, ircServer.domain, event.user_id);
            matrixRooms.push(mxRoom);
        }
        // already tracking channel, so just invite them.
        let invitePromises = matrixRooms.map((room) => {
            req.log.info("Inviting %s to room %s", event.user_id, room.getId());
            return this.ircBridge.getAppServiceBridge().getIntent().invite(room.getId(), event.user_id);
        });
        // check whether we should be force joining the IRC user
        for (let i = 0; i < matrixRooms.length; i++) {
            let m = matrixRooms[i];
            let userMustJoin = (key || ircServer.shouldSyncMembershipToIrc("incremental", m.getId()));
            if (userMustJoin) {
                // force join then break out (we only ever join once no matter how many
                // rooms the channel is bridged to)
                let bc = yield this.ircBridge.getBridgedClient(ircServer, event.user_id);
        ...
        if (!whoisNick) {
            yield this.ircBridge.sendMatrixAction(adminRoom, botUser, new MatrixAction("notice", "Format: '!whois nick|mxid'"), req);
            return;
        }
        if (whoisNick[0] === "@") {
            ...
            try {
                let noticeRes = new MatrixAction("notice", whoisClient ?
        ...
        req.log.info("%s wants whois info on %s on %s", event.user_id, whoisNick, ircServer.domain);
        let bridgedClient = yield this.ircBridge.getBridgedClient(ircServer, event.user_id);
        try {
        ...
        const msgText = yield this.quitUser(req, event.user_id, clientList, ircServer, "issued !quit command");
        if (msgText) {
        ...
        try {
            ...
            if (pass.length === 0) {
                notice = new MatrixAction("notice", "Format: '!storepass password' " +
                    "or '!storepass irc.server.name password'\n" + explanation);
            }
            else {
                yield this.ircBridge.getStore().storePass(userId, domain, pass);
                notice = new MatrixAction("notice", `Successfully stored password on ${domain}. ` + explanation);
            }
        }
        catch (err) {
            notice = new MatrixAction("notice", `Failed to store password: ${err.message}`);
        ...
        try {
            ...
            notice = new MatrixAction("notice", `Successfully removed password.`);
        }
        catch (err) {
            notice = new MatrixAction("notice", `Failed to remove password: ${err.message}`);
        }
        yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
        return;
    }
        ...
        try {
            ...
            let bridgedClient = yield this.ircBridge.getBridgedClient(currentServer, event.user_id);
        ...
        try {
            ...
            yield this.ircBridge.sendMatrixAction(adminRoom, botUser, new MatrixAction("notice", `BridgeVersion: ${BridgeVersion}`), req);
        }
        catch (err) {
        ...
        if (args.length === 0 || !USER_FEATURES.includes(args[0].toLowerCase())) {
            let notice = new MatrixAction("notice", "Missing or unknown feature flag. Must be one of: " + USER_FEATURES.join(", "));
            yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
            return;
        }
        const featureFlag = args[0];
        const features = yield this.ircBridge.getStore().getUserFeatures(event.user_id);
        if (!args[1]) {
        ...
        if (!["true", "false", "default"].includes(args[1].toLowerCase())) {
            let notice = new MatrixAction("notice", "Parameter must be either true, false or default.");
            yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
            return;
        }
        features[featureFlag] = args[1] === "default" ? undefined :
            args[1].toLowerCase() === "true";
        yield this.ircBridge.getStore().storeUserFeatures(event.user_id, features);
        let note = "";
        if (featureFlag === "mentions") {
            // We should invalidate caching for this user's channels.
            if (!this.ircBridge.ircHandler.invalidateCachingForUserId(event.user_id)) {
                note = " This bridge has disabled mentions, so this flag will do nothing.";
            }
        }
        let notice = new MatrixAction("notice", `Set ${featureFlag} to ${features[featureFlag]}.${note}`);
        yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
    }
    else {
        let notice = new MatrixAction("notice", "The command was not recognised. Available commands are listed by !help");
        yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
    }
});
MatrixHandler.prototype.quitUser = Promise.coroutine(function* (req, userId, clientList, ircServer, reason) {
    let clients = clientList;
    if (ircServer) {
        // Filter to get the clients for the [specified] server
        clients = clientList.filter((bridgedClient) => bridgedClient.server.domain === ircServer.domain);
    }
    if (clients.length === 0) {
    ...
    for (let i = 0; i < clients.length; i++) {
        ...
        if (bridgedClient.chanList.length === 0) {
            req.log.info(`Bridged client for ${userId} is not in any channels ` +
                `on ${bridgedClient.server.domain}`);
        }
        else {
            // Get all rooms that the bridgedClient is in
            let rooms = yield Promise.all(bridgedClient.chanList.map((channel) => {
                return this.ircBridge.getStore().getMatrixRoomsForChannel(bridgedClient.server, channel);
            }));
            // rooms is an array of arrays
            rooms = rooms.reduce((a, b) => { return a.concat(b); });
            let uniqueRoomIds = Array.from(new Set(rooms.map((matrixRoom) => matrixRoom.roomId)));
            for (let j = 0; j < uniqueRoomIds.length; j++) {
                let roomId = uniqueRoomIds[j];
                try {
                    yield this.ircBridge.getAppServiceBridge().getIntent().kick(roomId, bridgedClient.userId, reason);
                }
                catch (err) {
                    req.log.error(err);
                    req.log.warn(`Could not kick ${bridgedClient.userId} ` +
...
MatrixHandler.prototype._onJoin = Promise.coroutine(function* (req, event, user) {
    ...
    ircRooms = ircRooms.filter(function (room) {
        return room.server.shouldSyncMembershipToIrc(syncKind, event.room_id) || event._frontier;
    });
    ...
    ircRooms.forEach(function (room) {
        if (room.server.claimsUserId(user.getId())) {
            req.log.info("%s is a virtual user (claimed by %s)", user.getId(), room.server.domain);
            return;
        }
        ...
        promises.push(Promise.coroutine(function* () {
            ...
            let kickIntent;
            try {
                bridgedClient = yield self.ircBridge.getBridgedClient(room.server, user.getId(), (event.content || {}).displayname);
            }
            catch (e) {
            ...
            while (kickIntent) {
                try {
                    yield kickIntent.kick(event.room_id, user.getId(), `Connection limit reached for ${room.server.domain}. Please try again later.`);
                    ...
                    req.log.warn(`User was not kicked. Retrying in ${delay}ms. ${err}`);
            ...
            if (event.content.displayname !== bridgedClient.displayName) {
                ...
                let config = yield self.ircBridge.getStore().getIrcClientConfig(bridgedClient.userId, room.server.domain);
                if (room.server.allowsNickChanges() &&
                    config.getDesiredNick() === null) {
                    try {
                        const newNick = room.server.getNick(bridgedClient.userId, event.content.displayname);
...
MatrixHandler.prototype._onKick = Promise.coroutine(function* (req, event, kicker, kickee) {
    req.log.info("onKick %s is kicking/banning %s from %s", kicker.getId(), kickee.getId(), event.room_id);
    this._onMemberEvent(req, event);
    /*
    ...
    */
    ...
    if (kickeeClients.length === 0) {
        ...
        if (!kickerClient) {
            ...
            req.log.error("%s has no client instance to send kick from. Cannot kick.", kicker.getId());
            return;
        }
        ...
        for (let i = 0; i < ircRooms.length; i++) {
            ...
            kickerClient.kick(kickeeNick, ircRooms[i].channel, `Kicked by ${kicker.getId()}` +
        ...
        ircRooms.forEach(function (ircRoom) {
            ...
            promises.push(client.leaveChannel(ircRoom.channel, `Kicked by ${kicker.getId()} ` +
                (event.content.reason ? ` : ${event.content.reason}` : "")));
        });
        yield Promise.all(promises);
    }
    ...
    ircRooms.forEach((ircRoom) => {
        ...
        if (!ircRoom.server.shouldJoinChannelsIfNoUsers()) {
            if (ircRoom.server.domain) {
                // this = IrcBridge
                this.ircBridge.memberListSyncers[ircRoom.server.domain].checkBotPartRoom(ircRoom, req);
            }
        }
    });
...
MatrixHandler.prototype._onMessage = Promise.coroutine(function* (req, event) {
    ...
    for (let i = 0; i < servers.length; i++) {
        if (servers[i].claimsUserId(event.user_id)) {
            req.log.info("%s is a virtual user (claimed by %s)", event.user_id, servers[i].domain);
            return BridgeRequest.ERR_VIRTUAL_USER;
        }
    }
    ...
    if (this._processingInvitesForRooms[event.room_id + event.user_id]) {
        ...
        req.log.info("Finished holding event for %s in room %s", event.user_id, event.room_id);
    }
    if (this.ircBridge.getAppServiceUserId() === event.user_id) {
        // ignore messages from the bot
        return BridgeRequest.ERR_VIRTUAL_USER;
    ...
    let mxAction = MatrixAction.fromEvent(this.ircBridge.getAppServiceBridge().getClientFactory().getClientAs(), event, mediaUrl);
    ...
    ircRooms.forEach((ircRoom) => {
        ...
        if (ircRoom.getType() === "channel") {
            otherPromises.push(this.ircBridge.getStore().getMatrixRoomsForChannel(ircRoom.server, ircRoom.channel).then((otherMatrixRooms) => {
        ...
        if (!this.ircBridge.getIrcUserFromCache(ircRoom.server, event.user_id)) {
            promises.push(Promise.coroutine(function* () {
                ...
                try {
                    let res = yield self.ircBridge.getAppServiceBridge().getBot()
                        .getClient().getStateEvent(event.room_id, "m.room.member", event.user_id);
                ...
                let ircUser = yield self.ircBridge.getBridgedClient(ircRoom.server, event.user_id, displayName);
            ...
            promises.push(Promise.coroutine(function* () {
                let ircUser = yield self.ircBridge.getBridgedClient(ircRoom.server, event.user_id);
                yield self._sendIrcAction(req, ircRoom, ircUser, ircAction, event);
            })());
        }
    });
    ...
    Object.keys(otherMatrixRoomIdsToServers).forEach((roomId) => {
        ...
        promises.push(this.ircBridge.sendMatrixAction(new MatrixRoom(roomId), new MatrixUser(virtUserId), mxAction, req));
    });
    yield Promise.all(promises);
});
MatrixHandler.prototype._sendIrcAction = Promise.coroutine(function* (req, ircRoom, ircClient, ircAction, event) {
    ...
    if (result.content_uri) {
        ...
        let truncatedIrcAction = IrcAction.fromMatrixAction(MatrixAction.fromEvent(this.ircBridge.getAppServiceBridge().getClientFactory().getClientAs(), event, mediaUrl));
...
MatrixHandler.prototype._onAliasQuery = Promise.coroutine(function* (req, roomAlias) {
    ...
    let matrixRooms = yield this.ircBridge.getStore().getMatrixRoomsForChannel(channelInfo.server, channelInfo.channel);
    if (matrixRooms.length === 0) {
...
>>>>>>> origin/develop
        ...
        this.ircBridge.publicitySyncer.initModeForChannel(channelInfo.server, channelInfo.channel).catch((err) => {
            log.error(`Could not init mode for channel ${channelInfo.channel} on ` +
        ...
        yield this.ircBridge.getAppServiceBridge().getBot().getClient().createAlias(roomAlias, roomId);
    }
});
/**
...
MatrixHandler.prototype._textForReplyEvent = Promise.coroutine(function* (event, ircRoom) {
    ...
    if (sourceClient) {
        ...
        rplName = ghostName !== null ? ghostName : rplName.substr(1, Math.min(REPLY_NAME_MAX_LENGTH, rplName.indexOf(":") - 1));
    }
    return {
