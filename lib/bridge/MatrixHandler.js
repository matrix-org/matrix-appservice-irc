...
MatrixHandler.prototype._onAdminMessage = Promise.coroutine(function*(req, event, adminRoom) {
    ...
    if (cmd === "!help") {
        let helpCommands = {
            ...
            "!feature": {
                example: `!feature feature-name [true/false]`,
                summary: `Enable or disable a feature for your account.` +
                         `Will display if true/false not given.`,
            },
            "!quit": {
                example: `!quit`,
    ...
    if (cmd === "!nick") {
        ...
        try {
            ...
            yield this.ircBridge.sendMatrixAction(adminRoom,
                  botUser,
        ...
        if (!args[1]) {
            const val = yield features[featureFlag];
        ...
        if (!["true", "false"].includes(args[1].toLowerCase())) {
            let notice = new MatrixAction("notice",
        ...
        features[featureFlag] = args[1].toLowerCase() === "true";
        ...
        if (featureFlag === "mentions") {
             ...
             this.ircBridge.ircHandler.invalidateCachingForUserId(event.user_id);
        }
        let notice = new MatrixAction("notice",
            `Set ${featureFlag} to ${features[featureFlag]}`
        );
        yield this.ircBridge.sendMatrixAction(adminRoom, botUser, notice, req);
    }
    else {
...
MatrixHandler.prototype._textForReplyEvent = Promise.coroutine(function*(event, ircRoom) {
    ...
    if (sourceClient) {
        ...
        // If we failed to get a hostname, just make a guess of it.
