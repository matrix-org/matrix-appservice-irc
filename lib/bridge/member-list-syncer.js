...
function MemberListSyncer(appServiceBot, server, appServiceUserId, injectJoinFn) {
...
MemberListSyncer.prototype.getChannelsToJoin = Promise.coroutine(function*() {
    ...
    let promises = rooms.map(function(roomInfo) {
        return store.rooms.getIrcChannelsForRoomId(roomInfo.id).then(function(ircRooms) {
    ...
    if (matrixRooms.length === 0) {
        ...
        matrixRooms.forEach(function(matrixRoom) {
            ...
            for (var i = 0; i < syncableRooms.length; i++) {
                if (syncableRooms[i].id === matrixRoom.roomId) {
                    shouldPart = false;
                    break;
                }
            }
