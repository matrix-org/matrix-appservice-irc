...
class IrcUser extends RemoteUser {
    ...
    constructor(server, nick, isVirtual, password, username, userConfig) {
        super("-", {
    ...
    setUserId(userId) {
        this.setUserConfig("user_id", userId);
        this._regenerateId();
    }

    getUserConfig(key) {
        return this.get("user_config")[key];
    }

    setUserConfig(key, val) {
        this.get("user_config")[key] = val;
    }
    ...
    _regenerateId() { // FIXME
        ...
        this.id = userId ? this.server.domain + userId : this.server.domain + "~" + this.nick
    }
}
...
IrcUser.fromRemoteUser = function(server, remoteUser) {
    var ircUser = new IrcUser(
        ...
        remoteUser.get("password"), remoteUser.get("username"), remoteUser.get("user_config")
    );
    return ircUser;
};
