 ...
 * @param {BridgedClient} bridgedClient : The virtual client connection for this
 * use if they are not a real IRC user.
 * @param {string} password : The password to give to NickServ.
 * @param {string} username : The username of the client (for ident)
 */
function IrcUser(server, nick, bridgedClient, password, username) {
    this.protocol = "irc";
    this.isVirtual = Boolean(bridgedClient);
    this.server = server;
    this.nick = nick;
    this.client = bridgedClient;
