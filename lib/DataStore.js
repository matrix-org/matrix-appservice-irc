...
DataStore.prototype.storePass(userId, domain, pass) {
    return new Promise((resolve, reject) => {

        // Call once ever and put somewhere in the config?
        let salt = crypto.randomBytes(256);
        let iterations = 100000;
        let keylen = 512;

        crypto.pbkdf2(pass, salt, iterations, keylen, 'sha512', (err, key) => {
            if (err) {
                reject(err);
            }
            encryptedPass = key.toString('base64');

            let config = yield this.ircBridge.getStore().getClientConfig(userId, domain);
