...
DataStore.prototype.getIrcClient = function(userId, domain) {
    return this._userStore.getRemoteUsersFromMatrixId(userId).then((remoteUsers) => {
        let remoteUser = null;
        for (let i = 0; i < remoteUsers.length; i++) {
            if (remoteUsers[i].get("domain") === domain) {
        ...
        let server = this._serverMappings[remoteUser.get("domain")];
        return server ? IrcUser.fromRemoteUser(server, remoteUser) : null;
    });
};

DataStore.prototype.storeIrcClient = function(userId, ircUser) {
    log.info("Storing " + ircUser + " on behalf of " + userId);
    let matrixUser = new MatrixUser(userId);
    return this._userStore.linkUsers(matrixUser, ircUser);
};

DataStore.prototype.getIrcClientByUsername = function(domain, username) {
    return this._userStore.getByRemoteData({
        username: username,
        domain: domain
    }).then((remoteUsers) => {
        if (remoteUsers.length > 1) {
            log.error(
                "getIrcClientByUsername return %s results for %s on %s",
                remoteUsers.length, username, domain
            );
        }
        let user = remoteUsers[0];
        if (!user) {
            return null;
        }
        let server = this._serverMappings[user.get("domain")];
        if (!server) {
            return null;
        }
        return IrcUser.fromRemoteUser(server, user);
    });
};
