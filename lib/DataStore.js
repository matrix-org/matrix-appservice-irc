...
function DataStore(userStore, roomStore, pkeyPath) {
...
DataStore.prototype.storePass = Promise.coroutine(
    function*(userId, domain, pass, forcePlaintextStorage) {
        ...
        if (this._privateKey && !config._privateKey) {
            config.setPrivateKey(this._privateKey);
        }
