...
DataStore.prototype.getTrackedChannelsForServer = function(ircAddr) {
    return this._roomStore.getRemoteRooms({ domain: ircAddr }).then(function(rooms) {
        return rooms.map((r) => {
            return IrcRoom.fromRemoteRoom(r);
        }).filter((ircRoom) => {
            return ircRoom.getType() === "channel";
        }).map(function(ircRoom) {
            return ircRoom.getChannel();
        });
    });
};
