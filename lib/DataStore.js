...
DataStore.prototype.getIrcClient = function(userId, domain) {
    return this._userStore.getRemoteUser(domain + userId).then((remoteUser) => {
        if (!remoteUser) {
            return null;
        }
...
DataStore.prototype.storeIrcClient = function(userId, ircUser) {
    ...
    ircUser.setUserId(userId);
    return this._userStore.setRemoteUser(ircUser);
};

DataStore.prototype.getIrcClientByUsername = Promise.coroutine(
function*(domain, username) {
    let remoteUsers = yield this._userStore.getByRemoteData({
        username: username,
        domain: domain
    });
