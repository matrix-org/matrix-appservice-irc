...
var Scheduler = {
    ...
    reschedule: Promise.coroutine(function* (server, addedDelayMs, retryConnection, nick) {
        ...
        var promise = q.enqueue(`Scheduler.reschedule ${server.domain} ${nick}`, {
            fn: retryConnection,
            addedDelayMs: addedDelayMs
        });
        log.info(`Queued scheduled promise for ${server.domain} ${nick}` +
            (addedDelayMs > 0 ? ` with ${Math.round(addedDelayMs)}ms added delay` : ''));
        log.info(`Queue for ${server.domain} length = ${q._queue.length}`);
        return promise;
    }),
