...
export class IrcServer {
    ...
    public getHardCodedRoomIds() {
        ...
        channels.forEach((chan) => {
            this.config.mappings[chan].forEach((roomId: string) => {
                roomIds.add(roomId);
            });
        });
    ...
    public shouldSyncMembershipToIrc(kind: string, roomId: string) {
        return this._shouldSyncMembership(kind, roomId, true);
    }

    public shouldSyncMembershipToMatrix(kind: string, channel: string) {
        return this._shouldSyncMembership(kind, channel, false);
    }

    public _shouldSyncMembership(kind: string, identifier: string, toIrc: boolean) {
        ...
        if (toIrc) {
            ...
            this.config.membershipLists.rooms.forEach((r: any) => {
            ...
            this.config.membershipLists.channels.forEach((chan: any) => {
    ...
    public getNick(userId: string, displayName: string) {
        ...
        const possibleDisplayName = displayName ? displayName.replace(illegalChars, "") : undefined;
        const display = [possibleDisplayName, localpart].find((n) => Boolean(n));
        if (!display) {
    ...
    private static templateToRegex(template: string, literalVars: {[key: string]: string}, regexVars: {[key: string]: string}, suffix: string) {
