...
MemberListSyncer.prototype.getChannelsToJoin = Promise.coroutine(function* () {
    ...
    yield this.ircBridge.getStore().getIrcChannelsForRoomIds(roomIds).then((roomIdToIrcRoom) => {
        Object.keys(roomIdToIrcRoom).forEach((roomId) => {
            ...
            ircRooms.forEach((ircRoom) => {
                ...
                log.debug("%s should be joined because %s real Matrix users are in room %s", ircRoom.channel, roomInfoMap[roomId].realJoinedUsers.length, roomId);
    ...
    log.debug("getChannelsToJoin => %s should be synced: %s", channelsArray.length, JSON.stringify(channelsArray));
...
MemberListSyncer.prototype.checkBotPartRoom = Promise.coroutine(function* (ircRoom, req) {
    ...
    let matrixRooms = yield this.ircBridge.getStore().getMatrixRoomsForChannel(ircRoom.server, ircRoom.channel);
...
MemberListSyncer.prototype._getSyncableRooms = function () {
    ...
    let fetchRooms = Promise.coroutine(function* () {
        ...
        let pool = new QueuePool(50, Promise.coroutine(function* (roomId) {
            ...
            log.info("%s has %s real Matrix users and %s remote users (%s/%s)", roomId, roomInfo.realJoinedUsers.length, roomInfo.remoteJoinedUsers.length, roomInfoList.length, joinedRoomIds.length);
        }));
...
MemberListSyncer.prototype._joinMatrixUsersToChannels = function (rooms, server, injectJoinFn) {
    ...
    rooms.forEach(function (roomInfo) {
        if (!server.shouldSyncMembershipToIrc("initial", roomInfo.id)) {
            log.debug("Trimming room %s according to config rules (matrixToIrc=false)", roomInfo.id);
    ...
    const joinNextUser = () => {
        ...
        log.debug("Injecting join event for %s in %s (%s left) is_frontier=%s", entry.userId, entry.roomId, entries.length, entry.frontier);
        this._usersToJoin--;
        injectJoinFn(entry.roomId, entry.userId, entry.displayName, entry.frontier).timeout(server.getMemberListFloodDelayMs()).then(() => {
            joinNextUser();
        }, (err) => {
            joinNextUser();
        });
...
MemberListSyncer.prototype.leaveIrcUsersFromRooms = function (rooms, server) {
    log.info(`leaveIrcUsersFromRooms: storing member list info for ${rooms.length} ` +
...
MemberListSyncer.prototype.updateIrcMemberList = Promise.coroutine(function* (channel, names) {
    ...
    log.info(`updateIrcMemberList: Updating IRC member list for ${channel} with ` +
        `${this._memberLists.irc[channel].length} IRC nicks`);
    // Convert the IRC channels nicks to userIds
    let ircUserIds = this._memberLists.irc[channel].map((nick) => this.server.getUserIdFromNick(nick));
    // For all bridged rooms, leave users from matrix that are not in the channel
    let roomsForChannel = yield this.ircBridge.getStore().getMatrixRoomsForChannel(this.server, channel);
    ...
    roomsForChannel.forEach((matrixRoom) => {
        ...
        if (!(this._memberLists.matrix[roomId] &&
            this._memberLists.matrix[roomId].remoteJoinedUsers)) {
            return;
        }
        let usersToLeave = this._memberLists.matrix[roomId].remoteJoinedUsers.filter((userId) => {
    ...
    log.info(`updateIrcMemberList: Leaving ${promises.length} users as they are not in ${channel}.`);
...
MemberListSyncer.prototype.addToLeavePool = function (userIds, roomId, channel) {
    this._usersToLeave += userIds.length;
    this._leaveQueuePool.enqueue(roomId + " " + channel, {
        roomId,
        userIds
    });
};
