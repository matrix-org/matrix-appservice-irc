...
function IrcBridge(config, registration) {
    ...
    this._ircEventBroker = new IrcEventBroker(this._bridge, this._clientPool, this.ircHandler);
    ...
    this._debugApi = (config.ircService.debugApi.enabled ? new DebugApi(this, config.ircService.debugApi.port, this.ircServers, this._clientPool, registration.getAppServiceToken()) : null);
...
IrcBridge.prototype._initialiseMetrics = function () {
    ...
    this._bridge.registerBridgeGauges(() => {
        const remoteUsersByAge = new PrometheusMetrics.AgeCounters(this.config.ircService.metrics.remoteUserAgeBuckets || ["1h", "1d", "1w"]);
    ...
    metrics.addCollector(() => {
        this.ircServers.forEach((server) => {
            reconnQueue.set({ server: server.domain }, this._clientPool.totalReconnectsWaiting(server.domain));
...
IrcBridge.prototype.createBridgedClient = function (ircClientConfig, matrixUser, isBot) {
    ...
    if (!server) {
        throw new Error("Cannot create bridged client for unknown server " +
            ircClientConfig.getDomain());
    }
    return new BridgedClient(server, ircClientConfig, matrixUser, isBot, this._ircEventBroker, this._identGenerator, this._ipv6Generator);
};
IrcBridge.prototype.run = Promise.coroutine(function* (port) {
    ...
    this._dataStore = new DataStore(this._bridge.getUserStore(), this._bridge.getRoomStore(), pkeyPath, this.config.homeserver.domain);
    ...
    if (this.config.appService) {
        console.warn(`[DEPRECATED] Use of config field 'appService' is deprecated. Remove this
        ...
        this.appServiceUserId = ("@" + (this.config.appService.localpart ||
        ...
        this.appServiceUserId = ("@" + this.registration.getSenderLocalpart() + ":" +
            this.domain);
    }
    ...
    this.ircServers.forEach((server) => {
        ...
        this.memberListSyncers[server.domain] = new MemberListSyncer(this, this._bridge.getBot(), server, this.appServiceUserId, (roomId, joiningUserId, displayName, isFrontier) => {
            var req = new BridgeRequest(this._bridge.getRequestFactory().newRequest(), false);
...
IrcBridge.prototype._logMetric = function (req, outcome) {
    ...
    const timer = this._timers[isFromIrc ? "remote_request_seconds" : "matrix_request_seconds"];
    if (timer) {
    ...
    if (msgtype) {
        if (action.htmlText) {
            return intent.sendMessage(room.getId(), {
                msgtype: msgtype,
                body: (action.text || action.htmlText.replace(/(<([^>]+)>)/ig, "") // strip html tags
                ),
...
IrcBridge.prototype.matrixToIrcUser = function (user) {
    ...
    if (!ircInfo.server || !ircInfo.nick) {
        return Promise.reject(new Error("User ID " + user.getId() + " doesn't map to a server/nick"));
    }
...
IrcBridge.prototype._loginToServer = Promise.coroutine(function* (server) {
    ...
    if (!bridgedClient) {
        ...
        log.debug("Created new bot client for %s : %s (bot enabled=%s)", server.domain, bridgedClient._id, server.isBotEnabled());
    }
    var chansToJoin = [];
    if (server.isBotEnabled()) {
...
IrcBridge.prototype.getBridgedClient = Promise.coroutine(function* (server, userId, displayName) {
    ...
    let ircClientConfig = IrcClientConfig.newConfig(mxUser, server.domain, null);
    ...
    log.debug("Creating virtual irc user with nick %s for %s (display name %s)", ircClientConfig.getDesiredNick(), userId, displayName);
    ...
    try {
        ...
        log.error("Couldn't connect virtual user %s to %s : %s", ircClientConfig.getDesiredNick(), server.domain, JSON.stringify(err));
        throw err;
    }
});
IrcBridge.prototype.sendIrcAction = function (ircRoom, bridgedClient, action) {
    log.info("Sending IRC message in %s as %s (connected=%s)", ircRoom.channel, bridgedClient.nick, Boolean(bridgedClient.unsafeClient));
...
IrcBridge.prototype._onRoomUpgrade = Promise.coroutine(function* (oldRoomId, newRoomId) {
    ...
    if (bridgingEvent) {
        ...
        try {
            yield this._bridge.getIntent().sendStateEvent(newRoomId, "m.room.bridging", bridgingEvent.state_key, bridgingEvent.content);
    ...
    yield Promise.all(rooms.map((room) => {
        return this.getBotClient(room.getServer()).then((bot) => {
            ...
            this.memberListSyncers[room.getServer().domain].addToLeavePool(roomInfo.remoteJoinedUsers, oldRoomId, room);
        });
    }));
